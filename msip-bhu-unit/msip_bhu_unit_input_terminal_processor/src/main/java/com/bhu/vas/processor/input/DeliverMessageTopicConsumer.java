				...
				doSubscribeTopics(new PollIteratorNotify<ConsumerRecords<String, String>>() {
					...
					public void notifyComming(String consumerId, ConsumerRecords<String, String> records) {
						for (ConsumerRecord<String, String> record : records) {
							logger.info(String.format(LoggerFormatTemplate, record.topic(), 
									                  record.partition(), record.key(), 
									                  record.value(), record.offset(), consumerId));

							KafkaMsgObserverManager.DynaMsgCommingObserver.notifyMsgComming
