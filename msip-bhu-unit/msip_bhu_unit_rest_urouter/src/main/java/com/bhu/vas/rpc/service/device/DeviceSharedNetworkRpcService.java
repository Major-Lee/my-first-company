...
public class DeviceSharedNetworkRpcService implements IDeviceSharedNetworkRpcService {
	...
	@Override
	public RpcResponseDTO<Boolean> modifyNetworkConf(int uid, String ssid, int rate, List<String> macs){
		logger.info(String.format("modifyNetworkConf with uid[%s] ssid[%s] rate[%s] mac[%s]", uid, ssid, rate, macs));
		return deviceSharedNetworkUnitFacadeService.modifyNetworkConf(uid, ssid, rate, macs);
	}
	...
	public RpcResponseDTO<UserSnkPortalVTO> fetchUserSnks4Portal(int uid,String sharenetwork_type, String tpl) {
		logger.info(String.format("fetchUserSnks4Portal with uid[%s] snk_type[%s], tpl[%s]", uid,sharenetwork_type, tpl));
		return deviceSharedNetworkUnitFacadeService.fetchUserSnks4Portal(uid,sharenetwork_type, tpl);
	}
