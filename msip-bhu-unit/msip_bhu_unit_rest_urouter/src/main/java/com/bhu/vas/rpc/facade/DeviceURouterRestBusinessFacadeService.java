...
public class DeviceURouterRestBusinessFacadeService {
	 ...
	 * LoggerFactory.getLogger(DeviceRestBusinessFacadeService.class);
	 * 
	 ...
	 * 
	 * private static final String HANDSET_LOGIN_TYPE = "login"; private static
	 * final String HANDSET_LOGOUT_TYPE = "logout";
	 */
	...
	/*
	 * @Resource private HandsetDeviceService handsetDeviceService;
	 */
	...
	// @Resource
	// private WifiHandsetDeviceRelationMService
	// wifiHandsetDeviceRelationMService;
	...
	/**
	 * urouter 主入口界面数据 hd_list接口需要终端的所有数据 路由器上下行速率数据 绑定的路由器在线状态、终端个数、路由器个数、路由器名称
	 * 需要路由器版本号 插件配置信息或者信号强度
	 * 
	 * @param uid
	 * @param wifiId
	 * @return
	...
	public RpcResponseDTO<URouterEnterVTO> urouterEnter(Integer uid, String wifiId) {
		try {
			// WifiDevice device_entity =
			// uRouterDeviceFacadeService.validateDevice(wifiId);
			deviceFacadeService.validateUserDevice(uid, wifiId);
			// WifiDeviceSetting entity =
			// deviceFacadeService.validateDeviceSetting(wifiId);
			...
			// String peak_rate =
			// WifiDeviceRealtimeRateStatisticsStringService.getInstance().getPeak(wifiId);
	 ...
	 * @param start
	 * @param size
	 * @param filterWiredHandset
	 *            是否过滤掉有线的终端设备
	 *            由于有分页机制，按分页提取的数据如果移除部分元素可能导致分页的相关参数和结果集不符合，但是客户端由于没有传递分页，
	 *            所以可以忽略 //@param wireless wireless null 不区分有线无线 true 无线 false
	 *            有线
	 * @return modified by Edmond Lee for handset storage
	 */
	public RpcResponseDTO<Map<String, Object>> urouterHdList(Integer uid, String wifiId, int status, int start,
			int size, Boolean filterWiredHandset) {
		try {
			deviceFacadeService.validateUserDevice(uid, wifiId);
			...
			case HDList_Online_Status:
				presents = WifiDeviceHandsetUnitPresentSortedSetService.getInstance()
						.fetchOnlinePresentWithScores(wifiId, start, size);
				// total =
				// WifiDeviceHandsetUnitPresentSortedSetService.getInstance().presentOnlineSize(wifiId);
				break;
			...
			if (!presents.isEmpty()) {
				...
				List<String> handsetAlias = WifiDeviceHandsetAliasService.getInstance().pipelineHandsetAlias(uid,
						hd_macs);
				// List<HandsetDevice> hd_entitys =
				// handsetDeviceService.findByIds(hd_macs, true, true);
				// List<WifiHandsetDeviceMark> mark_entitys =
				// wifiHandsetDeviceMarkService.findByIds(mark_pks, true, true);
				if (!handsets.isEmpty()) {
					...
					for (Tuple tuple : presents) {
						...
						if (hd_entity != null) {
							...
							double rx_rate = Double.parseDouble(
									hd_entity.getData_rx_rate() == null ? "0" : hd_entity.getData_rx_rate());
							URouterHdVTO vto = BusinessModelBuilder.toURouterHdVTO(uid, tuple.getElement(), online,
									rx_rate, hd_entity, setting_dto, alia);
							vtos.add(vto);
							cursor++;
						}
	...
	public RpcResponseDTO<Map<String, Object>> urouterAllHdList(Integer uid, String wifiId, int start, int size,
		...
		try {
			...
			presents = WifiDeviceHandsetUnitPresentSortedSetService.getInstance().fetchPresentWithScores(wifiId, start,
					size);
	...
	public RpcResponseDTO<Map<String, String>> countOnlineByTimestamp(Integer uid, String wifiId, Long timestamp) {
		...
		try {
			...
			List<Object> counts = WifiDeviceHandsetUnitPresentSortedSetService.getInstance()
					.presentOnlineSizeWithScore(wifiIds, timestamp);
	...
	/*
	 * public static int daysOfTwo(Date fDate, Date oDate) { Calendar aCalendar
	 * = Calendar.getInstance(); aCalendar.setTime(fDate); int day1 =
	 * aCalendar.get(Calendar.DAY_OF_YEAR); aCalendar.setTime(oDate); int day2 =
	 * aCalendar.get(Calendar.DAY_OF_YEAR); return day2 - day1; }
	 */
	...
	private Map<String, List<WifiHandsetDeviceItemDetailMDTO>> buildHdDetailMap(List<HandsetLogDTO> recentLogs) {
		...
		for (HandsetLogDTO log : recentLogs) {
			...
			if (DateUtils.isSameDay(login, logout)) {
				...
				add2Result(result, date,
						new WifiHandsetDeviceItemDetailMDTO(login.getTime(), completed ? logout.getTime() : 0l, trb));
				// count++;
			} else {// 跨天
				...
				for (int i = 0; i <= days; i++) {
					if (i == 0) {
						...
						add2Result(result, date,
								new WifiHandsetDeviceItemDetailMDTO(login.getTime(), end.getTime(), 0l));
					} else if (i == days) {
						...
						add2Result(result, date, new WifiHandsetDeviceItemDetailMDTO(start.getTime(),
								completed ? logout.getTime() : 0l, trb));
					} else {
						Date current = DateTimeHelper.getDateDaysAfter(login, i);
						String date = DateTimeHelper.formatDate(current, DateTimeHelper.FormatPattern5);
						Date start = DateTimeHelper.getCertainDateStart(current);
						...
						add2Result(result, date,
								new WifiHandsetDeviceItemDetailMDTO(start.getTime(), end.getTime(), 0l));
					}
					// count++;
				}
			}
	 ...
	 * @param dto
	 */
	private void add2Result(Map<String, List<WifiHandsetDeviceItemDetailMDTO>> result, String date,
			WifiHandsetDeviceItemDetailMDTO dto) {
	...
	/**
	 * 获取终端详情 //修改为redis实现终端上下线日志 2015-12-11
	 * 
	 * @param uid
	 * @param wifiId
	 * @param mac
	...
	/*
	 * public RpcResponseDTO<URouterHdDetailVTO> urouterHdDetail(Integer uid,
	 * String wifiId, String mac) {
	 * 
	 * URouterHdDetailVTO vto = new URouterHdDetailVTO(); vto.setHd_mac(mac);
	 * vto.setMac(wifiId); try { WifiHandsetDeviceRelationMDTO
	 * wifiHandsetDeviceRelationMDTO =
	 * wifiHandsetDeviceRelationMService.getRelation(wifiId, mac);
	 * 
	 * if (wifiHandsetDeviceRelationMDTO != null) { HandsetDeviceDTO
	 * handsetDeviceDTO = HandsetStorageFacadeService.handset(mac); long
	 * total_rx_bytes = wifiHandsetDeviceRelationMDTO.getTotal_rx_bytes();
	 * 
	 * if (handsetDeviceDTO != null) { if (handsetDeviceDTO.wasOnline()) {
	 * //离线的时候会加进去 if (handsetDeviceDTO.getTx_bytes() != null ) { total_rx_bytes
	 * = total_rx_bytes + Long.parseLong(handsetDeviceDTO.getTx_bytes()); } } }
	 * 
	 * vto.setTotal_rx_bytes(String.valueOf(total_rx_bytes));
	 * 
	 * List<String> weeks = DateTimeExtHelper.getSevenDateOfWeek();
	 * 
	 * List<URouterHdTimeLineVTO> uRouterHdTimeLineVTOList = new
	 * ArrayList<URouterHdTimeLineVTO>();
	 * 
	 * 
	 * //集合中只有七天的在线记录 for (String key : weeks) { URouterHdTimeLineVTO
	 * uRouterHdTimeLineVTO = new URouterHdTimeLineVTO();
	 * uRouterHdTimeLineVTO.setDate(key); List<WifiHandsetDeviceItemDetailMDTO>
	 ...
	 * wifiHandsetDeviceRelationMDTO.getLogs();
	 * 
	 * getLogs(uRouterHdTimeLineVTOList, logs);
	 * 
	 * vto.setTimeline(uRouterHdTimeLineVTOList); } return
	 * RpcResponseDTOBuilder.builderSuccessRpcResponse(vto);
	 * 
	 * } catch (BusinessI18nCodeException bex){ return
	 * RpcResponseDTOBuilder.builderErrorRpcResponse(bex.getErrorCode()); } }
	 */
	 ...
	 */
	/*
	 * private List<URouterHdTimeLineVTO> getLogs(List<URouterHdTimeLineVTO>
	 * vtos, List<WifiHandsetDeviceItemLogMDTO> logs) { long currentTime =
	 * System.currentTimeMillis(); long currentZeroTime = getDateZeroTime(new
	 * Date()).getTime();
	 * 
	 * if (logs != null) { //WifiHandsetDeviceItemDetailMDTO dto = null;
	 * //List<WifiHandsetDeviceItemDetailMDTO> mdtos = null; String last_type =
	 * null; long last_ts = 0;
	 * 
	 * boolean flag = false; for (WifiHandsetDeviceItemLogMDTO log : logs) {
	 * 
	 * long ts = log.getTs(); long space = currentZeroTime - ts; int offset =
	 * (int)(space/(DAY_TIME_MILLION_SECOND)); if (space < 0) { offset = -1; }
	 * if (offset > 5) { //offset = 5; //7天外的数据直接忽略 if
	 * (HANDSET_LOGOUT_TYPE.equals(last_type)) { //如果最后一天数据是隔天数据
	 * 
	 * for (int i=6 ; i >=0 ;i--) { URouterHdTimeLineVTO vto = vtos.get(i);
	 * List<WifiHandsetDeviceItemDetailMDTO> mdtos_ = vto.getDetail(); if(mdtos_
	 * != null && !mdtos_.isEmpty()) { URouterHdTimeLineVTO last_vto =
	 * vtos.get(i); List<WifiHandsetDeviceItemDetailMDTO> last_mdtos =
	 * last_vto.getDetail(); WifiHandsetDeviceItemDetailMDTO last_dto =
	 * last_mdtos.get(last_mdtos.size() - 1);
	 * last_dto.setLogin_at(getDateZeroTime(
	 * DateTimeHelper.parseDate(last_vto.getDate(),
	 * DateTimeHelper.shortDateFormat)).getTime());
	 * last_vto.setDetail(last_mdtos); break; } } } break; }
	 * 
	 * String type = log.getType(); long rx_bytes = log.getRx_bytes();
	 * 
	 * // logger.info("offset[" + offset + "],type[" + type + "],last_type[" +
	 * last_type+"],ts[" + ts + "]"); // logger.info("spacetime[" + (last_ts
	 * -ts) + "]"); if (last_type == null) { //最新一条记录 //处理分割记录 if
	 * (HANDSET_LOGIN_TYPE.equals(type)) { flag = true; }
	 * 
	 * if (HANDSET_LOGOUT_TYPE.equals(type) && logs.size() == 1) {
	 * //仅有一条logout的脏数据 break; }
	 * 
	 * flag = filterDay(ts, currentTime, type,last_type, rx_bytes, vtos, offset,
	 * true, flag);
	 * 
	 * } else { //第二条数据开始
	 * 
	 * if (HANDSET_LOGOUT_TYPE.equals(type) &&
	 * HANDSET_LOGIN_TYPE.equals(last_type)) { //新的的登出记录 flag = filterDay(ts,
	 * last_ts, type, last_type, rx_bytes, vtos, offset, false, flag); }
	 * 
	 * if (HANDSET_LOGOUT_TYPE.equals(type) &&
	 * HANDSET_LOGOUT_TYPE.equals(last_type)) { //连续两条登出 //忽略记录 //模拟一条登入的记录 //
	 * last_type = "logout"; type = HANDSET_LOGIN_TYPE; flag = filterDay(ts,
	 * last_ts, type, last_type, rx_bytes, vtos, offset, false, flag);
	 * 
	 * last_type = HANDSET_LOGIN_TYPE; type = HANDSET_LOGOUT_TYPE; flag =
	 * filterDay(last_ts, last_ts, type, last_type, rx_bytes, vtos, offset,
	 * false, flag);
	 * 
	 * }
	 * 
	 * if (HANDSET_LOGIN_TYPE.equals(type) &&
	 * HANDSET_LOGOUT_TYPE.equals(last_type)) {
	 * 
	 * flag = filterDay(ts, last_ts, type, last_type, rx_bytes,vtos, offset,
	 * false, flag); } if (HANDSET_LOGIN_TYPE.equals(type) &&
	 * HANDSET_LOGIN_TYPE.equals(last_type)) { //忽略记录 //先模拟一条当前登出的记录 type =
	 * HANDSET_LOGOUT_TYPE; flag = filterDay(last_ts, last_ts, type, last_type,
	 * rx_bytes, vtos, getOffSet(ts, currentZeroTime), false, flag);
	 * 
	 * last_type = HANDSET_LOGOUT_TYPE; type = HANDSET_LOGIN_TYPE; // last_ts =
	 * ts; flag = filterDay(ts, last_ts, type, last_type, rx_bytes, vtos,
	 * getOffSet(ts, currentZeroTime), false, flag);
	 * 
	 * }
	 * 
	 * }
	 * 
	 * last_type = type; last_ts = ts;
	 * 
	 * 
	 * }
	 * 
	 ...
	 * 
	 * 
	 * private int getOffSet(long ts, long currentZeroTime) { long space =
	 * currentZeroTime - ts; int offset = (int)
	 * (space/(DAY_TIME_MILLION_SECOND)); if (space < 0) { offset = -1; } if
	 * (offset > 5) { offset = 5; } return offset; }
	 */
	/**
	 *
	 * 解析终端登入登出日志，每条日志只有时间戳和类型(login/logout) 1.正常流程 login -> logout -> login ->
	 * logout -> ... 2.出现跨天的情况 3.15分钟之内的登入登出合并
	 *
	 *
	 *
	 * @param ts
	 ...
	 */
	/*
	 * private boolean filterDay(long ts, long last_ts, String type, String
	 * last_type, long rx_bytes, List<URouterHdTimeLineVTO> vtos, int offset,
	 * boolean first, boolean flag) {
	 * 
	 * //如果当前在线，当前时间与上一次登录时间相隔数天 String tsZeroStr =
	 * DateTimeHelper.formatDate(new Date(ts), DateTimeHelper.shortDateFormat);
	 * 
	 * long ts_zero_at = DateTimeHelper.getDateLongTime(tsZeroStr,
	 * DateTimeHelper.shortDateFormat);
	 * 
	 * long spaceTime = last_ts - ts_zero_at; //获取终端连续在线的时间段j天 int j =
	 * (int)spaceTime / (DAY_TIME_MILLION_SECOND); if (j >= 6) { j = 6; }
	 * 
	 * logger.info("ts["+ts+"],last_ts["+last_ts+"], type["+type+"], last_type["
	 * +last_type+"], offset["+offset+"],j["+j+"]"); try {
	 * 
	 * if (j == 0) { //当天记录 URouterHdTimeLineVTO vto = vtos.get(offset + 1);
	 * //更新logs List<WifiHandsetDeviceItemDetailMDTO> mdtos = vto.getDetail();
	 * if (mdtos == null) { mdtos = new
	 * ArrayList<WifiHandsetDeviceItemDetailMDTO>(); }
	 * 
	 * WifiHandsetDeviceItemDetailMDTO dto = null;
	 * 
	 * // if (type.equals("login") && last_type.equals("logout")) { //正常流程 if
	 * (HANDSET_LOGIN_TYPE.equals(type)) { //正常流程 if (first) { // last_type ==
	 * null dto = new WifiHandsetDeviceItemDetailMDTO(); dto.setLogin_at(ts);
	 * dto.setLogout_at(0); dto.setRx_bytes(0); mdtos.add(dto); } else { dto =
	 * mdtos.get(mdtos.size() - 1); dto.setLogin_at(ts); } }
	 * 
	 * if (HANDSET_LOGOUT_TYPE.equals(type)) { //正常流程
	 * 
	 * if (first) { //last_type == null dto = new
	 * WifiHandsetDeviceItemDetailMDTO(); dto.setLogout_at(ts);
	 * dto.setRx_bytes(rx_bytes); mdtos.add(dto); } else { if (last_ts - ts <
	 * IGNORE_LOGIN_TIME_SPACE) { //小于15分钟的记录 //忽略操作 // logger.info(
	 * "ignore 15 min" + (ts - last_ts)); dto = mdtos.get(mdtos.size() - 1); if
	 * (flag) { //如果不在线的话就累加 dto.setRx_bytes(0); } else {
	 * dto.setRx_bytes(dto.getRx_bytes() + rx_bytes); } } else { dto = new
	 * WifiHandsetDeviceItemDetailMDTO(); dto.setLogout_at(ts);
	 * dto.setRx_bytes(rx_bytes); mdtos.add(dto); flag = false; }
	 * 
	 * } } // logger.info("[mdtos]" + mdtos.size()); vto.setDetail(mdtos);
	 * 
	 * }
	 * 
	 * if (j > 0) {
	 ...
	 * WifiHandsetDeviceItemDetailMDTO dto = null;
	 * 
	 * if (first) { //隔天的第一条记录 URouterHdTimeLineVTO vto = vtos.get(offset + 1);
	 * //更新logs List<WifiHandsetDeviceItemDetailMDTO> mdtos = vto.getDetail();
	 * if (mdtos == null) { mdtos = new
	 * ArrayList<WifiHandsetDeviceItemDetailMDTO>(); }
	 * 
	 * if (HANDSET_LOGOUT_TYPE.equals(type)) { dto = new
	 * WifiHandsetDeviceItemDetailMDTO(); dto.setLogout_at(ts);
	 * dto.setRx_bytes(rx_bytes); mdtos.add(dto); }
	 * 
	 * if (HANDSET_LOGIN_TYPE.equals(type)) { //忽略 dto = new
	 * WifiHandsetDeviceItemDetailMDTO(); dto.setLogin_at(ts); mdtos.add(dto); }
	 * 
	 * }
	 * 
	 * if (HANDSET_LOGOUT_TYPE.equals(type) &&
	 * HANDSET_LOGIN_TYPE.equals(last_type)) { //如果上一次正常退出
	 * 
	 * //有隔天记录的拆分第一条记录 >>> URouterHdTimeLineVTO vto = vtos.get(offset + 1);
	 * //更新logs // logger.info("date===date[" + vto.getDate() + "]"); //
	 * List<WifiHandsetDeviceItemDetailMDTO> mdtos = vto.getLogs(); //
	 * List<WifiHandsetDeviceItemDetailMDTO> mdtos = vto.getDetail(); if (mdtos
	 * == null) { mdtos = new ArrayList<WifiHandsetDeviceItemDetailMDTO>(); }
	 * 
	 * dto = new WifiHandsetDeviceItemDetailMDTO(); dto.setLogout_at(ts);
	 * dto.setRx_bytes(rx_bytes); mdtos.add(dto); vto.setDetail(mdtos); }
	 * 
	 * 
	 * //隔天记录 //补齐上一天的最后一条login记录为00:00:00，当天的第一条记录11:59:59 if
	 * (HANDSET_LOGIN_TYPE.equals(type) &&
	 * HANDSET_LOGOUT_TYPE.equals(last_type)) { ////隔天仍在线
	 * 
	 * //如果j >1 的时候 offset >= 0 for (int i = 1; i< j + 1 ; i++) { // >>> j == 1
	 * logger.info("iiiii===" + i); URouterHdTimeLineVTO vto_ = vtos.get(offset
	 * - (j - i )); List<WifiHandsetDeviceItemDetailMDTO> mdtos_ =
	 * vto_.getDetail(); if (mdtos_ == null) { mdtos_ = new
	 * ArrayList<WifiHandsetDeviceItemDetailMDTO>(); }
	 * WifiHandsetDeviceItemDetailMDTO dto_ = mdtos_.get(mdtos_.size() - 1);
	 * long login_at_zero = DateTimeHelper.parseDate(vto_.getDate(),
	 * DateTimeHelper.shortDateFormat).getTime();
	 * dto_.setLogin_at(login_at_zero); //补齐零点登入 vto_.setDetail(mdtos_); // <<<
	 * 
	 ...
	 * dto.setLogout_at(login_at_zero - 1); //补齐零点登出 dto.setRx_bytes(rx_bytes);
	 * 
	 * // logger.info("set ts ==" + ts + ",i===" + i); if (i == j) {
	 * dto.setLogin_at(ts); //如果最后一次的话添加一个登录时间 // logger.info("set login_ ts =="
	 * + ts); } // if (offset + 1 + i >= 6) { // dto.setLogin_at(ts);
	 * //如果最后一次的话添加一个登录时间 // mdtos.add(dto); // vto.setDetail(mdtos); // break;
	 * // }
	 * 
	 * 
	 * mdtos.add(dto); vto.setDetail(mdtos); // <<< j == 1 }
	 * 
	 * }
	 * 
	 * //vto.setLogs(mdtos);
	 * 
	 * //有隔天记录的拆分第一条记录 <<< } }catch(Exception e) { e.printStackTrace(); } return
	 * flag; }
	 */

	/*
	 * private Date getDateZeroTime(Date date) { Calendar cal =
	 * Calendar.getInstance(); cal.setTime(date); int hour =
	 * cal.get(Calendar.HOUR_OF_DAY); int minute = cal.get(Calendar.MINUTE); int
	 * second = cal.get(Calendar.SECOND); //时分秒（毫秒数） long millisecond =
	 * hour*60*60*1000 + minute*60*1000 + second*1000; //凌晨00:00:00
	...
	/**
	 * 获取设备的实时速率 a:如果存在数据null 下发实时速率指令 b:如果存在数据waiting 什么都不做 c:如果存在数据非ab 返回数据
	 * 
	 * @param uid
	 * @param wifiId
	 * @return
	...
	public RpcResponseDTO<URouterPeakSectionsDTO> urouterPeakSectionFetch(Integer uid, String wifiId) {
		try {
			 ...
			 * getPeakSections(wifiId); if(rets != null && rets.length == 2){
			 * if(rets[0] != null){
			 * vto.setRxs((List<WifiDeviceRxPeakSectionDTO>)rets[0]); }
			 * if(rets[1] != null){
			 * vto.setTxs((List<WifiDeviceTxPeakSectionDTO>)rets[1]); } }
			 */
			URouterPeakSectionsDTO dto = WifiDeviceRealtimeRateStatisticsStringService.getInstance()
					.getPeakSections(wifiId);
	 ...
	 */
	/*
	 * public String fetchPeakRate(String wifiId){ List<String> result =
	 * WifiDeviceRealtimeRateStatisticsStringService.getInstance().getPeak(
	 * wifiId); String peak_rate = result.get(0); String peak_rate_waiting =
	 * result.get(1); //如果waiting没有标记 则发送指令查询
	 * if(StringUtils.isEmpty(peak_rate_waiting)){ //调用异步消息下发网速测试指令
	 * deliverMessageService.sendQueryDeviceSpeedFetchActionMessage(wifiId); }
	 * return peak_rate; }
	 */
		...
		try {
			...
			if (acl_dto != null) {
				...
				if (block_hd_macs_all != null && !block_hd_macs_all.isEmpty()) {
					...
					// List<WifiHandsetDeviceMarkPK> mark_pks =
					// BusinessModelBuilder.toWifiHandsetDeviceMarkPKs(wifiId,
					// block_hd_macs);
					// List<HandsetDevice> hd_entitys =
					// handsetDeviceService.findByIds(block_hd_macs, true,
					// true);
					if (!block_hd_macs.isEmpty()) {
						List<HandsetDeviceDTO> handsets = HandsetStorageFacadeService.handsets(wifiId, block_hd_macs);
						List<String> handsetAlias = WifiDeviceHandsetAliasService.getInstance()
								.pipelineHandsetAlias(uid, block_hd_macs);
						vtos = new ArrayList<URouterHdVTO>();
						// List<WifiHandsetDeviceMark> mark_entitys =
						// wifiHandsetDeviceMarkService.findByIds(mark_pks,
						// true, true);
						WifiDeviceSettingDTO setting_dto = entity.getInnerModel();
						int cursor = 0;
						for (String block_hd_mac : block_hd_macs) {
							...
							URouterHdVTO vto = BusinessModelBuilder.toURouterHdVTO(uid, block_hd_mac, false, 0,
									handsets.get(cursor), setting_dto, alia);
							vtos.add(vto);
							cursor++;
						}
					}
	...
	public RpcResponseDTO<Boolean> urouterUpdPluginWifisniffer(Integer uid, String wifiId, boolean on) {
		try {
			 ...
			 * CMDBuilder.builderDeviceWifiSnifferSetting(wifiId, on ?
			 * WifiDeviceHelper.WifiSniffer_Start_Sta_Sniffer :
			 * WifiDeviceHelper.WifiSniffer_Stop_Sta_Sniffer));
			 */
			return RpcResponseDTOBuilder.builderSuccessRpcResponse(true);
		} catch (BusinessI18nCodeException bex) {
			return RpcResponseDTOBuilder.builderErrorRpcResponse(bex.getErrorCode(), bex.getPayload());
	...
	public RpcResponseDTO<DeviceUsedStatisticsDTO> urouterDeviceUsedStatusQuery(Integer uid, String wifiId) {
		try {
			...
			if (needNewRequestAndMarker) {
				deliverMessageService.sendWifiCmdsCommingNotifyMessage(
						wifiId, /*
								 * 0,OperationCMD.QueryDeviceUsedStatus.getNo(),
								 */
	...
	/**
	 * 插件数据 终端上线通知 终端wifi定时开关 终端wifi终端探测
	 * 
	...
	protected void urouterPlugins4TerminalOnline(WifiDevice wifiDevice, UserSettingState user_setting_entity,
		...
		// 终端上线设置数据
		UserTerminalOnlineSettingDTO uto_dto = user_setting_entity
				.getUserSetting(UserTerminalOnlineSettingDTO.Setting_Key, UserTerminalOnlineSettingDTO.class);
		// 定时开关设置数据
		UserWifiTimerSettingDTO uwt_dto = user_setting_entity.getUserSetting(UserWifiTimerSettingDTO.Setting_Key,
				UserWifiTimerSettingDTO.class);
		uwt_dto.setEnable(uwt_dto.wifiCurrentEnable());
		// 周边探测设置数据
		WifiSinfferSettingVTO uws_vto = new WifiSinfferSettingVTO();
		UserWifiSinfferSettingDTO uws_dto = user_setting_entity.getUserSetting(UserWifiSinfferSettingDTO.Setting_Key,
				UserWifiSinfferSettingDTO.class);

		if (uws_dto != null) {
			uws_vto.setOn(uws_dto.isOn());
			// 获取最近12小时出现的终端数量
			/*
			 * long current_ts = System.currentTimeMillis(); long hours12ago_ts
			 * = current_ts - (12 * 3600 * 1000l); long recent12hours_count =
			 * TerminalRecentSortedSetService.getInstance().sizeByScore(
			 * user_setting_entity.getId(), hours12ago_ts, current_ts);
		...
		/*
		 * UserVistorWifiSettingDTO uvw_dto =
		 * user_setting_entity.getUserSetting(UserVistorWifiSettingDTO.
		 * Setting_Key, UserVistorWifiSettingDTO.class); if(uvw_dto == null){
		 * uvw_dto = new UserVistorWifiSettingDTO();
		 * uvw_dto.setVw(ParamVapVistorWifiDTO.builderDefault(wifiDevice==null?
		 * true:WifiDeviceHelper.isWorkModeRouter(wifiDevice.getWork_mode())));
		 * //throw new
		 * BusinessI18nCodeException(ResponseErrorCode.COMMON_DATA_NOTEXIST);
		 * }else{
		 * uvw_dto.setC(WifiDeviceVisitorService.getInstance().countAuthPresent(
		 * user_setting_entity.getId())); }
		 */
		if (uto_dto != null)
	...
	public RpcResponseDTO<URouterModeVTO> urouterLinkMode(Integer uid, String wifiId) {
		try {
			deviceFacadeService.validateUserDevice(uid, wifiId);
			// WifiDeviceSetting setting_entity =
			// deviceFacadeService.validateDeviceSetting(wifiId);
			...
			if (acl_dto != null) {
				...
				if (hmacs != null && !hmacs.isEmpty()) {
					...
					for (String dto_hmac : hmacs) {
						...
						/*
						 * HandsetDeviceDTO dto = handsets.get(i); if (dto !=
						 * null) { nvto.setN(dto.getDhcp_name()); }
						 */
	...
	public RpcResponseDTO<URouterDeviceConfigMutilVTO> urouterConfigsSupportMulti(Integer uid, String dmac) {
		try {
			...
			// 获取正常的vap
			List<WifiDeviceSettingVapDTO> normal_vaps = DeviceHelper.getUrouterDeviceVapWithNames(setting_dto,
					DeviceHelper.NormalVapNames);
			...
			// String[] powerAndRealChannel =
			// DeviceHelper.getURouterDevicePowerAndRealChannel(setting_dto);
			// vto.setPower(Integer.parseInt(powerAndRealChannel[0]));
			// vto.setReal_channel(Integer.parseInt(powerAndRealChannel[1]));
			// admin密码
			WifiDeviceSettingUserDTO admin_user_dto = DeviceHelper.getURouterDeviceAdminUser(setting_dto);
	...
	public RpcResponseDTO<List<URouterHdHostNameVTO>> terminalHostnames(Integer uid, String dmac, String hmacs) {
		try {
			...
			List<HandsetDeviceDTO> handsets = HandsetStorageFacadeService.handsets(dmac,
					ArrayHelper.toList(macs_array));
			// List<HandsetDevice> entitys =
			// handsetDeviceService.findByIds(ArrayHelper.toList(macs_array),
			// true, true);
			int cursor = 0;
	...
	public RpcResponseDTO<Map<String, Object>> urouterWSRecent(Integer uid, String mac, int start, int size) {
		try {
			 ...
			 * RpcResponseDTOBuilder.builderSuccessRpcResponse(null); }
			 * 
			 * deviceFacadeService.validateUserDevice(uid, mac);
			 * 
			 * List<URouterWSRecentVTO> vto_list = null; //当前时间 long current_ts
			 * = System.currentTimeMillis(); //12小时之前的时间 //long hours12_ago_ts =
			 * current_ts - (12 * 3600 * 1000l); long hours12_ago_ts = 0;
			 * //System.out.println("step2"); long count =
			 * TerminalRecentSortedSetService.getInstance().sizeByScore(mac,
			 * hours12_ago_ts, current_ts); if(count == 0){ return
			 * RpcResponseDTOBuilder.builderSuccessRpcResponse(null); }
			 * //System.out.println("step3:"+count); Set<Tuple> tuples =
			 * TerminalRecentSortedSetService.getInstance().
			 * fetchTerminalRecentByScoreWithScores(mac, hours12_ago_ts,
			 * current_ts, start, size);
			 * 
			 * vto_list = new ArrayList<URouterWSRecentVTO>(); for(Tuple tuple :
			 * tuples){ URouterWSRecentVTO vto = new URouterWSRecentVTO();
			 * vto.setHd_mac(tuple.getElement());
			 * vto.setLast_snifftime(Double.valueOf(tuple.getScore()).longValue(
			 * ));
			 * vto.setTt(MacDictParserFilterHelper.prefixMactch(tuple.getElement
			 * (),true,false)); vto_list.add(vto); }
			 */
	...
	public RpcResponseDTO<Map<String, Object>> urouterWSNeighbour(Integer uid, String mac, int start, int size) {
		try {
			 ...
			 * 
			 * long count =
			 * TerminalHotSortedSetService.getInstance().terminalHotSize(mac);
			 * if(count == 0){ return
			 * RpcResponseDTOBuilder.builderSuccessRpcResponse(null); }
			 * Set<Tuple> tuples = TerminalHotSortedSetService.getInstance().
			 * fetchTerminalHotWithScores(mac, start, size);
			 * 
			 * List<URouterWSHotVTO> vto_list = new
			 * ArrayList<URouterWSHotVTO>();
			 * 
			 * String[] hd_macs = BusinessModelBuilder.toElementsArray(tuples);
			 * //List<String> last_time_strings =
			 * TerminalLastTimeStringService.getInstance().getMulti(mac,
			 * hd_macs); List<UserTerminalFocusDTO> focus_dtos =
			 * UserTerminalFocusHashService.getInstance().fetchUserTerminalFocus
			 * (uid, hd_macs); int cursor = 0; for(Tuple tuple : tuples){
			 ...
			 * vto.setTt(MacDictParserFilterHelper.prefixMactch(hd_macs[cursor],
			 * true,false)); UserTerminalFocusDTO focus_dto =
			 * focus_dtos.get(cursor); if(focus_dto != null){
			 * vto.setNick(focus_dto.getNick());
			 * vto.setFocus(focus_dto.isFocus()); } vto_list.add(vto); cursor++;
			 * }
			 */
	...
	public RpcResponseDTO<Boolean> urouterWSFocus(Integer uid, String hd_mac, boolean focus) {
		try {
			// UserTerminalFocusHashService.getInstance().setFocusValue(uid,
			// hd_mac, focus);
			return RpcResponseDTOBuilder.builderSuccessRpcResponse(true);
		} catch (BusinessI18nCodeException bex) {
			return RpcResponseDTOBuilder.builderErrorRpcResponse(bex.getErrorCode(), bex.getPayload());
	...
	public RpcResponseDTO<Boolean> urouterWSNick(Integer uid, String hd_mac, String nick) {
		try {
			// UserTerminalFocusHashService.getInstance().setNickValue(uid,
			// hd_mac, nick);
			return RpcResponseDTOBuilder.builderSuccessRpcResponse(true);
		} catch (BusinessI18nCodeException bex) {
			return RpcResponseDTOBuilder.builderErrorRpcResponse(bex.getErrorCode(), bex.getPayload());
	 ...
	 * @return
	 */
	public RpcResponseDTO<Map<String, Object>> urouterWSDetails(Integer uid, String mac, String hd_mac, int start,
			int size) {
		try {
			/*
			 * deviceFacadeService.validateUserDevice(uid, mac);
			 * 
			 * long count = TerminalDetailRecentSortedSetService.getInstance().
			 * terminalDetailRecentSize(mac, hd_mac); if(count == 0){ return
			 * RpcResponseDTOBuilder.builderSuccessRpcResponse(null); }
			 * Set<String> rets =
			 * TerminalDetailRecentSortedSetService.getInstance().
			 * fetchTerminalDetailRecent(mac, hd_mac, start, size); if(rets ==
			 * null || rets.isEmpty()){ return
			 * RpcResponseDTOBuilder.builderSuccessRpcResponse(null); }
			 * List<TerminalDetailDTO> dtos = new
			 * ArrayList<TerminalDetailDTO>();
			 * 
			 * int index = 0; for(String ret : rets){
			 * if(!StringUtils.isEmpty(ret)){ TerminalDetailDTO dto =
			 * JsonHelper.getDTO(ret, TerminalDetailDTO.class);
			 * //设备报送周边探测有可能出现只有上线，无下线消息的情况 此处判断如果不是最新探测到的细节 其他细节均显示为离线 if(index
			 * > 0){ dto.setState(WifistasnifferItemRddto.State_Offline); if
			 * (dto.getDuration() == 0) { //处理1s内的上下线为1s dto.setDuration(1); }
			 * }else{ //针对最新一条数据的无下线消息情况 此处终端探测的上线时间 超过timeout时间 则认为是离线 long
			 * life_time = System.currentTimeMillis() - dto.getSnifftime();
			 * if(WifistasnifferItemRddto.State_Offline_TimeoutMs - life_time <=
			 * 0){ dto.setState(WifistasnifferItemRddto.State_Offline); } }
			 * 
			 * dtos.add(dto); } index++; }
			 */
	...
	public RpcResponseDTO<URouterWSCommunityVTO> urouterWSCommunity(Integer uid, String mac) {
		try {
			 ...
			 * deviceFacadeService.validateUserDevice(uid, mac);
			 * 
			 * Map<String, String> communityCountByTypes =
			 * TerminalDeviceTypeCountHashService.getInstance().getAll(mac);
			 * if(communityCountByTypes == null ||
			 * communityCountByTypes.isEmpty()){ return
			 * RpcResponseDTOBuilder.builderSuccessRpcResponse(null); }
			 * URouterWSCommunityVTO vto =
			 * WifiStasnifferHelper.communityType(communityCountByTypes); return
			 * RpcResponseDTOBuilder.builderSuccessRpcResponse(vto);
			 */
	 ...
	 *            设备型号
	 * @param pt
	 *            (push type 针对ios的不同证书的参数)
	 * @return
	 */
	public RpcResponseDTO<Boolean> urouterUserMobileDeviceRegister(Integer uid, String d, String dt, String dm,
	...
	public RpcResponseDTO<URouterAdminPasswordVTO> urouterAdminPassword(Integer uid, String wifiId) {
		try {
			...
			if (wifiDeviceSetting != null) {
				...
				WifiDeviceSettingUserDTO wifiDeviceSettingUserDTO = DeviceHelper
						.getURouterDeviceAdminUser(wifiDeviceSettingDTO);

				if (wifiDeviceSettingUserDTO != null) {
					uRouterAdminPasswordVTO
							.setPassword(JNIRsaHelper.jniRsaDecryptHexStr(wifiDeviceSettingUserDTO.getPassword_rsa()));
				}
	...
	public RpcResponseDTO<URouterVapPasswordVTO> urouterVapPassword(Integer uid, String wifiId) {
		try {
			...
			if (wifiDeviceSetting != null) {
				WifiDeviceSettingDTO wifiDeviceSettingDTO = wifiDeviceSetting.getInnerModel();
				// List<WifiDeviceSettingVapDTO> wifiDeviceSettingVapDTOList =
				// wifiDeviceSettingDTO.getVaps();
				...
				uRouterVapPasswordVTO.setPassword(JNIRsaHelper.jniRsaDecryptHexStr(frist_vap_dto.getAuth_key_rsa()));
	...
	public RpcResponseDTO<URouterVisitorListVTO> urouterVisitorList(Integer uid, String wifiId, int start, int size) {
		...
		Set<Tuple> presents = WifiDeviceHandsetUnitPresentSortedSetService.getInstance()
				.fetchOnlinePresentWithScores(wifiId, start, size);
	...
	// 认证离线
	public RpcResponseDTO<URouterVisitorListVTO> urouterVisitorListOffline(Integer uid, String wifiId, int start,
			int size) {
	...
	// 访客网络未认证仅连接上线的列表
	public RpcResponseDTO<URouterVisitorListVTO> urouterVisitorListOnline(Integer uid, String wifiId, int start,
			int size) {
	...
	// 未认证 认证 认证离线
	public RpcResponseDTO<URouterVisitorListVTO> urouterVisitorListAll(Integer uid, String wifiId, int start,
			int size) {

		Set<Tuple> allPresents = WifiDeviceHandsetUnitPresentSortedSetService.getInstance()
	...
	private URouterVisitorListVTO builderURouterVisitorListVTO(Set<Tuple> presents, Integer uid, String wifiId,
		...
		/*
		 * UserSettingState settingState =
		 * userSettingStateService.getById(wifiId);
		 * 
		 * if (settingState != null) { UserVistorWifiSettingDTO vistorwifi =
		 * settingState.getUserSetting(UserVistorWifiSettingDTO.Setting_Key,
		 * UserVistorWifiSettingDTO.class); if (vistorwifi != null) { if
		 * (vistorwifi.getVw() == null ) { vto.setN(""); } else {
		...
		if (!presents.isEmpty()) {
			...
			// List<Object> handsetScores =
			// WifiDeviceVisitorService.getInstance().pipelineAllPresentScores(wifiId,
			// hd_macs_array);
			List<HandsetDeviceDTO> handsets = HandsetStorageFacadeService.handsets(wifiId, hd_macs);
