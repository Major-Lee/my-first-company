...
import com.bhu.statistics.util.cache.ListTranscoder;
import com.bhu.statistics.util.cache.ObjectsTranscoder;
import com.bhu.statistics.util.cache.SerializeTranscoder;
...
public class JedisClientImpl implements IJedisClient {
	...
	/**
	 * 通过key 和offset 从指定的位置开始将原先value替换下标从0开始,offset表示从offset下标开始替换
	 * 如果替换的字符串长度过小则会这样
	 * 
	 * @param key
	 * @param str
	...
	public String setObject(String key, Object value) {
		...
		try {
			...
			SerializeTranscoder s=new ObjectsTranscoder();
			byte [] values=s.serialize(value);
	...
	public <M> String setList(String key, List<M> list) {
		...
		try {
			...
			byte [] keys=key.getBytes();
			@SuppressWarnings("rawtypes")
			SerializeTranscoder s=new ListTranscoder();
			byte [] values=s.serialize(list);
			return jedis.set(keys, values);
		} catch (Exception e) {
			pool.returnBrokenResource(jedis);
			e.printStackTrace();
	...
	public <T> List<T> getList(String key, Class<T> c) {
		...
		try {
			...
			byte[] value=jedis.get(keys);
			@SuppressWarnings("unchecked")
			List<T> t= (List<T>) s.deserialize(value);
			return t;
		} catch (Exception e) {
			pool.returnBrokenResource(jedis);
			e.printStackTrace();
