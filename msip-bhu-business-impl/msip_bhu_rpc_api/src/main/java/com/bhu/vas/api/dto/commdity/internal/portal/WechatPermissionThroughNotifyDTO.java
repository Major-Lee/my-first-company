...
/**
 * 微信认证通过扣款成功后,数据写入约定的redis中进行发货通知
 * @author tangzichao
 *
 */
@SuppressWarnings("serial")
public class WechatPermissionThroughNotifyDTO implements PermissionThroughNotifyDTO, Serializable{
	//订单id
	...
	public String getPermissionNotifyType() {
		return PermissionThroughNotifyType.WechatPermissionNotify.getPrefix();
	}
...
/*	public static SMSPermissionThroughNotifyDTO from(Order order, Commdity commdity, User bindUser){
	...
	public static WechatPermissionThroughNotifyDTO from(Order order, String ait_time, User bindUser){
		if(order == null || ait_time == null) return null;
		WechatPermissionThroughNotifyDTO requestDeliverNotifyDto = new WechatPermissionThroughNotifyDTO();
		requestDeliverNotifyDto.setOrderid(order.getId());
		requestDeliverNotifyDto.setMac(order.getMac());
		requestDeliverNotifyDto.setUmac(order.getUmac());
		requestDeliverNotifyDto.setContext(order.getContext());
