...
public class AsyncMsgHandleService {
	 ...
	 * 5:统计增量 wifi设备的daily启动次数增量 (backend) 6:wifi设备增量索引
	 * 
	 * @param message
	 * @throws Exception
	 */
	public void wifiDeviceOnlineHandle(String message) throws Exception {
		...
		if (wifiDevice != null) {
				...
				if (BusinessRuntimeConfiguration.Device_SharedNetwork_Default_Start) {
					if (WifiDeviceHelper.autoDeviceSecureSharedNetworkStrategy(wifiDevice.getOrig_swver())) {
						...
						if(sharedNetwork != null){
							...
							/*
							 * if(!sharedNetwork.isOn() && !sharedNetwork.isDs()
							 * && psn != null){//如果是关闭的 并且设备未响应的ds=false
							 * 重新下发开启指令 logger.info(String.format(
							 * "Device SharedNetwork Model[%s]",
							 * JsonHelper.getJSONString(psn)));
							 * sharedNetwork.turnOn(psn);
							 * sharedNetworksFacadeService.
							 * getWifiDeviceSharedNetworkService().update(
							 * sharedNetwork); //更新索引，下发指令
							 * wifiDeviceIndexIncrementService.
							 * sharedNetworkUpdIncrement(dto.getMac(),
							 * psn.getNtype(),psn.getTemplate());
							 * psn.switchWorkMode(WifiDeviceHelper.
							 * isWorkModeRouter(wifiDevice.getWork_mode()));
							 * //生成下发指令 String sharedNetworkCMD =
							 * CMDBuilder.autoBuilderCMD4Opt(OperationCMD.
							 * ModifyDeviceSetting,OperationDS.
							 * DS_SharedNetworkWifi_Start, dto.getMac(),
							 * -1,JsonHelper.getJSONString(psn),
							 * deviceCMDGenFacadeService);
							 * payloads.add(sharedNetworkCMD); }
							 */
						}
			...
			// 设备统计
			deviceFacadeService.deviceStatisticsOnline(
					new DeviceStatistics(dto.getMac(), dto.isNewWifi(), new Date(dto.getLast_login_at())),
					DeviceStatistics.Statis_Device_Type);
		}
	...
	public void wifiDeviceModuleOnlineHandle(String message) throws Exception {
		...
		if (wifiDevice != null) {
			...
			// dto.getOrig_vap_module());
		}
		logger.info(String.format("AnsyncMsgBackendProcessor wifiDeviceModuleOnlineHandle message[%s] successful",
				message));
	}
	...
	public void afterDeviceOnlineThenCmdDown(String mac, boolean needLocationQuery, List<String> payloads) {
		...
		/*
		 * //设备持久指令分发 try{ List<String> persistencePayloads =
		 * deviceFacadeService.fetchWifiDevicePersistenceCMD(mac);
		 ...
		 * +persistencePayloads.size()); } }catch(Exception ex){
		 * ex.printStackTrace(System.out); }
		 */
	...
	/**
	 * a:如果设备是新上线的 1：wifi设备基础信息更新(backend) 2：wifi设备在线状态Redis更新(backend) 3:统计增量
	 * wifi设备的daily新增设备或活跃设备增量 (backend) 4:统计增量 wifi设备的daily启动次数增量(backend)
	 * b:如果设备本身是在线的 do nothing 5:增量索引
	 * 
	 * @throws Exception
	 */
	public void cmupWithWifiDeviceOnlinesHandle(String message) throws Exception {
		...
		CMUPWithWifiDeviceOnlinesDTO cmupWithOnlinesDto = JsonHelper.getDTO(message,
				CMUPWithWifiDeviceOnlinesDTO.class);
		...
		logger.info(String.format("AnsyncMsgBackendProcessor cmupWithWifiDeviceOnlinesHandle message[%s] successful",
				message));
	}
	 ...
	 */
	/*
	 * public void cmupWithWifiDeviceOnlinesHandle(String message) throws
	 * Exception{ logger.info(String.format(
	 * "AnsyncMsgBackendProcessor cmupWithWifiDeviceOnlinesHandle message[%s]",
	 * message));
	 * 
	 * CMUPWithWifiDeviceOnlinesDTO cmupWithOnlinesDto =
	 * JsonHelper.getDTO(message, CMUPWithWifiDeviceOnlinesDTO.class);
	 * List<WifiDeviceDTO> dtos = cmupWithOnlinesDto.getDevices(); if(dtos !=
	 * null && !dtos.isEmpty()){ List<String> ids = new ArrayList<String>();
	 * for(WifiDeviceDTO dto : dtos){ ids.add(dto.getMac().toLowerCase()); }
	 * //List<WifiDevice> entitys = wifiDeviceService.findByIdsAndOnline(ids,
	 * false); List<WifiDevice> entitys = wifiDeviceService.findByIds(ids, true,
	 * true); //新上线的设备列表(非新注册) List<WifiDevice> entityNewOnlines = new
	 * ArrayList<WifiDevice>(); //新上线的并且是新注册的设备列表 List<WifiDevice>
	 * entityNewRegisters = new ArrayList<WifiDevice>(); //不在线的设备的ids
	 * List<String> offline_ids = new ArrayList<String>(); int cursor = 0;
	 * for(WifiDevice entity : entitys){ if(entity != null &&
	 * entity.isOnline()){ continue; } WifiDeviceDTO dto = dtos.get(cursor);
	 * if(entity == null){
	 * entityNewRegisters.add(BusinessModelBuilder.wifiDeviceDtoToEntity(dto));
	 * }else{ BeanUtils.copyProperties(dto, entity);
	 * entityNewOnlines.add(entity); }
	 * offline_ids.add(dto.getMac().toLowerCase()); cursor++; } //3:统计增量
	 * wifi设备的daily新增设备增量 int incr_statistics_news = 0; //3:统计增量
	 * wifi设备的daily活跃设备增量 int incr_statistics_active = 0; //4:统计增量
	 * wifi设备的daily启动次数增量 int incr_statistics_accesscount = 0; //有新上线的设备(非新注册)
	 * int newOnline_length = entityNewOnlines.size(); if(newOnline_length > 0){
	 * incr_statistics_accesscount += newOnline_length; //今天的时间 Date today = new
	 * Date(); for(WifiDevice entity: entityNewOnlines){
	 * //如果最后的登录时间和今天不一样，说明今天是第一次登录
	 * if(!DateTimeHelper.isSameDay(entity.getLast_reged_at(), today)){
	 * incr_statistics_active++; } entity.setLast_reged_at(new Date());
	 * entity.setOnline(true); } //1：wifi设备基础信息更新
	 * wifiDeviceService.updateAll(entityNewOnlines); } //新上线的并且是新注册的设备列表 int
	 * newRegister_length = entityNewRegisters.size(); if(newRegister_length >
	 * 0){ incr_statistics_accesscount += newRegister_length;
	 * incr_statistics_news += newRegister_length; //1：wifi设备基础信息更新
	 * wifiDeviceService.insertAll(entityNewRegisters); }
	 * 
	 * //2:wifi设备在线状态Redis更新 if(!offline_ids.isEmpty()){
	 * WifiDevicePresentService.getInstance().addPresents(offline_ids,
	 * cmupWithOnlinesDto.getCtx()); }
	 * 
	 * //3:统计增量 wifi设备的daily新增设备增量 if(incr_statistics_news > 0){
	 * DailyStatisticsHashService.getInstance().incrStatistics(BusinessKeyDefine
	 * .Statistics. DailyStatisticsDeviceInnerPrefixKey,
	 * DailyStatisticsDTO.Field_News, incr_statistics_news); } //3:统计增量
	 * wifi设备的daily活跃设备增量 if(incr_statistics_active > 0){
	 * DailyStatisticsHashService.getInstance().incrStatistics(BusinessKeyDefine
	 * .Statistics. DailyStatisticsDeviceInnerPrefixKey,
	 * DailyStatisticsDTO.Field_Actives, incr_statistics_active); } //4:统计增量
	 * wifi设备的daily启动次数增量 if(incr_statistics_accesscount > 0){
	 * DailyStatisticsHashService.getInstance().incrStatistics(BusinessKeyDefine
	 * .Statistics. DailyStatisticsDeviceInnerPrefixKey,
	 * DailyStatisticsDTO.Field_AccessCount, incr_statistics_accesscount); }
	 * //5:增量索引
	 * wifiDeviceIndexIncrementService.cmupWithWifiDeviceOnlinesIndexIncrement(
	 * entitys); } logger.info(String.format(
	 * "AnsyncMsgBackendProcessor cmupWithWifiDeviceOnlinesHandle message[%s] successful"
	 * , message)); }
	 */
	...
	public void wifiDeviceOfflineHandle(String message) throws Exception {
		...
		if (entity != null) {
			...
			{// 修改为redis实现终端上下线日志 2015-12-11
				...
				HandsetStorageFacadeService.wifiDeviceIllegalOffline(wifiId,
						deviceFacadeService.allHandsetDoOfflines(wifiId));
			}
			// 5:统计增量 wifi设备的daily访问时长增量
			/*
			 * if(dto.getLast_login_at() > 0){ long uptime = dto.getTs() -
			 * dto.getLast_login_at(); if(uptime > 0){
			 * deviceFacadeService.deviceStatisticsOffline(uptime,
			 * DeviceStatistics.Statis_Device_Type);
			 * 
			 * String entity_uptime = entity.getUptime(); BigInteger bi = new
			 * BigInteger(String.valueOf(uptime)); BigInteger entity_bi = null;
			 * if(!StringUtils.isEmpty(entity_uptime)){ entity_bi = new
			 * BigInteger(entity_uptime); bi = bi.add(entity_bi); }
	...
	/**
	 * 移动设备上线 3:移动设备连接wifi设备的接入记录(非流水) (backend) 4:移动设备连接wifi设备的流水log (backend)
	 * removed 5:wifi设备接入移动设备的接入数量增量 (backend) 6:统计增量
	 * 移动设备的daily新增用户或活跃用户增量(backend) 7:统计增量 移动设备的daily启动次数增量(backend)
	 * 
	 * @param message
	 */
	public void handsetDeviceOnlineHandle(String message) {
		...
		/*
		 * //3:移动设备连接wifi设备的接入记录(非流水) int result_status =
		 * wifiHandsetDeviceRelationMService.addRelation(dto.getWifiId(),
		 * dto.getMac(),new Date(dto.getLogin_ts())); //如果接入记录是新记录
		 * 表示移动设备第一次连接此wifi设备 if(result_status ==
		 * WifiHandsetDeviceRelationMService.AddRelation_Insert){
		...
		// int result_status =
		// HandsetStorageFacadeService.wifiDeviceHandsetOnline(dto.getWifiId(),
		// dto.getMac(),
		// dto.getLogin_ts());//wifiHandsetDeviceRelationMService.addRelation(dto.getWifiId(),
		// dto.getMac(),new Date(dto.getLogin_ts()));
		/*
		 * //如果接入记录是新记录 表示移动设备第一次连接此wifi设备 if(result_status ==
		 * HandsetLogDTO.Element_NewHandset){ //5:wifi设备接入移动设备的接入数量增量
		 * wifiHandsetDeviceLoginCountMService.incrCount(dto.getWifiId()); }
		 */
		// 4:移动设备连接wifi设备的流水log 增值平台首页面 此部分数据统计暂时移除 edmond 20160121
		// BusinessWifiHandsetRelationFlowLogger.doFlowMessageLog(dto.getWifiId(),
		// dto.getMac(), dto.getLogin_ts());
		// 终端统计 由于去除了增值平台首页面 此部分数据统计暂时移除 edmond 20160121
		// deviceFacadeService.deviceStatisticsOnline(new
		// DeviceStatistics(dto.getMac(), dto.isNewHandset(), new
		// Date(dto.getLast_login_at())),
		// DeviceStatistics.Statis_HandsetDevice_Type);

		// 如果是urouter设备 才会发push
		if (deviceFacadeService.isURouterDevice(dto.getWifiId())) {
			boolean terminal_notify_push_mark = businessCacheService.getQTerminalPushNotifyCacheByQ(dto.getWifiId(),
					dto.getMac());
			if (!terminal_notify_push_mark) {
				...
				if (push_successed) {
					logger.info(String.format(
							"AnsyncMsgBackendProcessor handsetDeviceOnlineHandle push mac[%s] hd_mac[%s] result[%s] ",
		...
		logger.info(
				String.format("AnsyncMsgBackendProcessor handsetDeviceOnlineHandle message[%s] successful", message));
	}

	/**
	 * 移动设备下线 3:统计增量 移动设备的daily访问时长增量 (backend) removed
	 * 
	 * @param message
	 */
	public void handsetDeviceOfflineHandle(String message) {
		...
		// HandsetDeviceOfflineDTO dto = JsonHelper.getDTO(message,
		// HandsetDeviceOfflineDTO.class);
		// 修改为redis实现终端上下线日志 2015-12-11
		// wifiHandsetDeviceRelationMService.offlineWifiHandsetDeviceItems(dto.getWifiId(),
		// dto.getMac(), dto.getTx_bytes(), dto.getTs());
		// HandsetStorageFacadeService.wifiDeviceHandsetOffline(dto.getWifiId(),
		// dto.getMac(), dto.getTx_bytes(), dto.getTs());

		// 3:统计增量 移动设备的daily访问时长增量 增值平台首页面 此部分数据统计暂时移除 edmond 20160121
		/*
		 * if(!StringUtils.isEmpty(dto.getUptime())){
		 * deviceFacadeService.deviceStatisticsOffline(Long.parseLong(dto.
		 * getUptime()), DeviceStatistics.Statis_HandsetDevice_Type); }
		 */

		logger.info(
				String.format("AnsyncMsgBackendProcessor handsetDeviceOfflineHandle message[%s] successful", message));
	}
	...
	public void handsetDeviceSyncHandle(String message) {
		...
		/*
		 * HandsetDeviceSyncDTO syncDto = JsonHelper.getDTO(message,
		 * HandsetDeviceSyncDTO.class);
		 * 
		 * for (HandsetDeviceDTO dto : syncDto.getDtos()) { //修改为redis实现终端上下线日志
		 * 2015-12-11
		 * //wifiHandsetDeviceRelationMService.addRelation(syncDto.getMac(),
		 * dto.getMac(),new Date(dto.getTs()));
		 * HandsetStorageFacadeService.wifiDeviceHandsetOnline(syncDto.getMac(),
		 * dto.getMac(),
		 * dto.getTs());//wifiHandsetDeviceRelationMService.addRelation(dto.
		 * getWifiId(), dto.getMac(),new Date(dto.getLogin_ts())); }
		 */
	}
	...
	public void handsetDeviceVisitorAuthorizeOnline(String message) {
		...
		HandsetDeviceVisitorAuthorizeOnlineDTO dto = JsonHelper.getDTO(message,
				HandsetDeviceVisitorAuthorizeOnlineDTO.class);
		if (deviceFacadeService.isURouterDevice(dto.getWifiId())) {
			...
			if (push_successed) {
				logger.info(String.format(
						"AnsyncMsgBackendProcessor handsetDeviceVisitorAuthorizeOnline push mac[%s] hd_mac[%s] result[%s] ",
						dto.getWifiId(), dto.getMac(), push_successed));
				// businessCacheService.storeQTerminalPushNotifyCacheResult(dto.getWifiId(),
				// dto.getMac());
			}
	...
	/**
	 * 移动设备连接状态sync 1:清除wifi设备对应handset在线列表redis 并重新写入 (backend) 2:移动设备基础信息更新
	 * (backend) 3:移动设备连接wifi设备的接入记录(非流水) (backend) 4:移动设备连接wifi设备的流水log
	 * (backend) 5:wifi设备接入移动设备的接入数量 (backend) 6:统计增量
	 * 移动设备的daily新增用户或活跃用户增量(backend) 7:统计增量 移动设备的daily启动次数增量(backend)
	 * 
	 * @param message
	 ...
	 * public void handsetDeviceSyncHandle(String message){
	 * logger.info(String.format(
	 * "AnsyncMsgBackendProcessor handsetDeviceSyncHandle message[%s]",
	 * message));
	 * 
	 * HandsetDeviceSyncDTO sync_dto = JsonHelper.getDTO(message,
	 * HandsetDeviceSyncDTO.class); String wifiId = sync_dto.getMac();
	 * if(!StringUtils.isEmpty(wifiId)){
	 * 
	 * List<HandsetDeviceDTO> dtos = sync_dto.getDtos(); if(dtos != null &&
	 * !dtos.isEmpty()){ List<String> ids = new ArrayList<String>();
	 * for(HandsetDeviceDTO dto : dtos){ ids.add(dto.getMac().toLowerCase()); }
	 * //终端统计模型 List<DeviceStatistics> ds = new ArrayList<DeviceStatistics>();
	 * 
	 * List<HandsetDeviceDTO> handsets =
	 * HandsetStorageFacadeService.handsets(ids); int cursor = 0;
	 * for(HandsetDeviceDTO handset : handsets){ if(handset != null &&
	 * handset.wasOnline()){ continue; } HandsetDeviceDTO dto =
	 * dtos.get(cursor); if(handset == null){ ds.add(new
	 * DeviceStatistics(dto.getMac(), true)); //entity =
	 * BusinessModelBuilder.handsetDeviceDtoToEntity(dto);
	 * //handset.setLast_wifi_id(wifiId); //entityNewRegisters.add(entity);
	 * }else{ ds.add(new DeviceStatistics(dto.getMac(), new
	 * Date(handset.getTs()))); dto.setDhcp_name(handset.getDhcp_name());
	 * dto.setData_tx_rate(handset.getData_tx_rate());
	 * dto.setData_rx_rate(handset.getData_rx_rate());
	 * BeanUtils.copyProperties(dto, entity,
	 * HandsetDeviceDTO.copyIgnoreProperties);
	 * if(!StringUtils.isEmpty(dto.getDhcp_name())){
	 * entity.setHostname(dto.getDhcp_name()); } entity.setLast_login_at(new
	 * Date()); entity.setLast_wifi_id(wifiId); entity.setOnline(true);
	 * entityNewOnlines.add(entity); } String handsetId =
	 * dto.getMac().toLowerCase(); //1:wifi设备对应handset在线列表redis 重新写入
	 * //WifiDeviceHandsetPresentSortedSetService.getInstance().addPresent(
	 * wifiId, handsetId, sync_dto.getTs()); //long rx_rate =
	 * StringUtils.isEmpty(entity.getData_rx_rate()) ? 0 :
	 * Long.parseLong(entity.getData_rx_rate());
	 * WifiDeviceHandsetPresentSortedSetService.getInstance().addOnlinePresent(
	 * wifiId, handsetId,
	 * handset.fetchData_rx_rate_double());//entity.getData_rx_rate_double());
	 * //3:移动设备连接wifi设备的接入记录(非流水) int result_status =
	 * wifiHandsetDeviceRelationMService.addRelation(wifiId, handsetId, new
	 * Date(sync_dto.getTs())); //如果接入记录是新记录 表示移动设备第一次连接此wifi设备 if(result_status
	 * == WifiHandsetDeviceRelationMService.AddRelation_Insert){
	 * //5:wifi设备接入移动设备的接入数量增量
	 * wifiHandsetDeviceLoginCountMService.incrCount(wifiId); }
	 * //4:移动设备连接wifi设备的流水log
	 * BusinessWifiHandsetRelationFlowLogger.doFlowMessageLog(wifiId, handsetId,
	 * sync_dto.getTs()); cursor++; } //有新上线的设备(非新注册)
	 * if(!entityNewOnlines.isEmpty()){ //2:移动设备基础信息更新
	 * handsetDeviceService.updateAll(entityNewOnlines); } //新上线的并且是新注册的设备列表
	 * if(!entityNewRegisters.isEmpty()){ //2:移动设备基础信息更新
	 * handsetDeviceService.insertAll(entityNewRegisters); }
	 * HandsetStorageFacadeService.handsetsComming(dtos); //终端统计
	 * deviceFacadeService.deviceStatisticsOnlines(ds,
	 * DeviceStatistics.Statis_HandsetDevice_Type); } }
	 * logger.info(String.format(
	 * "AnsyncMsgBackendProcessor handsetDeviceSyncHandle message[%s] successful"
	 * , message)); }
	 */
	/*
	 * public void handsetDeviceSyncHandle(String message){
	 ...
	 * HandsetDeviceSyncDTO sync_dto = JsonHelper.getDTO(message,
	 * HandsetDeviceSyncDTO.class); String wifiId = sync_dto.getMac();
	 * if(!StringUtils.isEmpty(wifiId)){ //1:清除wifi设备对应handset在线列表redis
	 * deviceFacadeService.allHandsetDoOfflines(sync_dto.getMac());
	 * //WifiDeviceHandsetPresentSortedSetService.getInstance().clearPresents(
	 * sync_dto.getMac());
	 * //WifiDeviceHandsetPresentSortedSetService.getInstance().
	 * clearOnlinePresents(sync_dto.getMac());
	 * 
	 * List<HandsetDeviceDTO> dtos = sync_dto.getDtos(); if(dtos != null &&
	 * !dtos.isEmpty()){ List<String> ids = new ArrayList<String>();
	 * for(HandsetDeviceDTO dto : dtos){ ids.add(dto.getMac().toLowerCase()); }
	 * //新上线的设备列表(非新注册) List<HandsetDevice> entityNewOnlines = new
	 * ArrayList<HandsetDevice>(); //新上线的并且是新注册的设备列表 List<HandsetDevice>
	 * entityNewRegisters = new ArrayList<HandsetDevice>(); //终端统计模型
	 * List<DeviceStatistics> ds = new ArrayList<DeviceStatistics>();
	 * 
	 * List<HandsetDevice> entitys = handsetDeviceService.findByIds(ids, true,
	 * true); int cursor = 0; for(HandsetDevice entity : entitys){ if(entity !=
	 * null && entity.isOnline()){ continue; } HandsetDeviceDTO dto =
	 * dtos.get(cursor); if(entity == null){ ds.add(new
	 * DeviceStatistics(dto.getMac(), true));
	 * 
	 * entity = BusinessModelBuilder.handsetDeviceDtoToEntity(dto);
	 * entity.setLast_wifi_id(wifiId); entityNewRegisters.add(entity);
	 * 
	 * // HandsetDevice handset =
	 * BusinessModelBuilder.handsetDeviceDtoToEntity(dto); //
	 * handset.setLast_wifi_id(wifiId); // entityNewRegisters.add(handset);
	 * }else{ ds.add(new DeviceStatistics(dto.getMac(),
	 * entity.getLast_login_at()));
	 * 
	 * BeanUtils.copyProperties(dto, entity,
	 * HandsetDeviceDTO.copyIgnoreProperties);
	 * if(!StringUtils.isEmpty(dto.getDhcp_name())){
	 ...
	 * Long.parseLong(entity.getData_rx_rate());
	 * WifiDeviceHandsetPresentSortedSetService.getInstance().addOnlinePresent(
	 * wifiId, handsetId, entity.getData_rx_rate_double());
	 * //3:移动设备连接wifi设备的接入记录(非流水) int result_status =
	 * wifiHandsetDeviceRelationMService.addRelation(wifiId, handsetId, new
	 * Date(sync_dto.getTs())); //如果接入记录是新记录 表示移动设备第一次连接此wifi设备 if(result_status
	 * == WifiHandsetDeviceRelationMService.AddRelation_Insert){
	 ...
	 * handsetDeviceService.insertAll(entityNewRegisters); }
	 * 
	 * //终端统计 deviceFacadeService.deviceStatisticsOnlines(ds,
	 * DeviceStatistics.Statis_HandsetDevice_Type); } }
	 * logger.info(String.format(
	 * "AnsyncMsgBackendProcessor handsetDeviceSyncHandle message[%s] successful"
	 * , message)); }
	 ...
	 * logger.info(String.format(
	 * "AnsyncMsgBackendProcessor wifiDeviceSettingModify message[%s]",
	 * message));
	 * 
	 * WifiDeviceSettingModifyDTO dto = JsonHelper.getDTO(message,
	 * WifiDeviceSettingModifyDTO.class); //String payload =
	 * "<dev><sys><config><ITEM sequence=\""
	 * +entity.getInnerModel().getSequence()+
	 * "\"/></config></sys><wifi><vap><ITEM name=\"wlan0\" radio=\"wifi0\" ssid=\"urouter-lwh\" auth=\"open\" enable=\"enable\" acl_type=\"deny\" acl_name=\"blackList\" guest_en=\"disable\"/><ITEM name=\"wlan2\" radio=\"wifi0\" ssid=\"BHU2\" auth=\"open\" enable=\"disable\" acl_type=\"deny\" acl_name=\"blackList\" guest_en=\"disable\"/><ITEM name=\"wlan3\" radio=\"wifi0\" ssid=\"BHU3\" auth=\"open\" enable=\"disable\" acl_type=\"deny\" acl_name=\"blackList\" guest_en=\"disable\"/></vap><acllist><ITEM name=\"blackList\" macs=\"\" /></acllist></wifi></dev>"
	 * ; //String payload = "<dev><sys><config><ITEM sequence=\""
	 * +entity.getInnerModel().getSequence()+
	 * "\"/></config></sys></dev><dev><wifi><vap><ITEM name=\"wlan0\" radio=\"wifi0\" ssid=\"urouter-lwh\" auth=\"open\" enable=\"enable\" acl_type=\"deny\" acl_name=\"blackList1\" guest_en=\"disable\"/><ITEM name=\"wlan2\" radio=\"wifi0\" ssid=\"BHU2\" auth=\"open\" enable=\"disable\" acl_type=\"deny\" acl_name=\"blackList1\" guest_en=\"disable\"/><ITEM name=\"wlan3\" radio=\"wifi0\" ssid=\"BHU3\" auth=\"open\" enable=\"disable\" acl_type=\"deny\" acl_name=\"blackList1\" guest_en=\"disable\"/></vap><acllist><ITEM name=\"blackList1\" macs=\"11:11:11:11:11:11\" /></acllist></wifi></dev>"
	 * ; DaemonHelper.deviceSettingModify(dto.getMac(), dto.getPayload(),
	 * daemonRpcService);
	 * 
	 * logger.info(String.format(
	 * "AnsyncMsgBackendProcessor wifiDeviceSettingModify message[%s] successful"
	 * , message)); }
	 */
	 ...
	 * if(!dto.isNewWifi()){
	 * //workmode切换只支持uRouter，并且只能在router-ap和bridge-ap之间互相切换 //判定workmode是否变更
	 * if(StringUtils.isNotEmpty(dto.getO_wmode()) &&
	 * StringUtils.isNotEmpty(dto.getN_wmode())){
	 * if(!dto.getO_wmode().equals(dto.getN_wmode())){
	 ...
	 * BusinessMarkerService.getInstance().deviceWorkmodeChangedMarker(dto.
	 * getMac(), WifiDeviceHelper.SwitchMode_Bridge2Router); }
	 * if(WifiDeviceHelper.WorkMode_Bridge.equals(dto.getN_wmode())){
	 * BusinessMarkerService.getInstance().deviceWorkmodeChangedMarker(dto.
	 * getMac(), WifiDeviceHelper.SwitchMode_Bridge2Router); } } } }
	 */
	/**
	 * 设备配置查询后续动作处理 1、设备重置情况下操作（考虑设备模式切换）如果是设备重置则直接清除设备模式切换标记，
	 * 如果不是设备重置状态则需要判定是否需要进行模式切换操作下发一些指令 2、uRouter 设备才进行周边探测是否继续重复下发
	 * 
	 * @param message
	 * @throws Exception
	 */
	public void wifiDeviceSettingQuery(String message) throws Exception {
		...
		/*
		 * if(dto.isDeviceURouter()){//deviceFacadeService.isURouterDevice(dto.
		 * getMac())){ if(DeviceHelper.RefreashDeviceSetting_Normal ==
		 * dto.getRefresh_status()){ //判断周边探测是否开启 如果开启 再次下发开启指令 UserSettingState
		 * settingState = userSettingStateService.getById(dto.getMac());
		 * if(settingState != null){ UserWifiSinfferSettingDTO wifiSniffer =
		 * settingState.getUserSetting(UserWifiSinfferSettingDTO.Setting_Key,
		 * UserWifiSinfferSettingDTO.class); if(wifiSniffer != null &&
		 * wifiSniffer.isOn()){ //if(cmdPayloads == null) cmdPayloads = new
		 * ArrayList<String>();
		...
		if (DeviceHelper.RefreashDeviceSetting_RestoreFactory == dto.getRefresh_status()) {
			...
			{// 在设备重置后的共享网络配置,给此设备下发此用户的共享网络配置 modify by Edmond Lee 20160322
				 ...
				 * wifiDeviceService.getById(dto.getMac()); if(wifiDevice !=
				 * null){
				 * if(WifiDeviceHelper.deviceSharedNetworkStrategy(wifiDevice.
				 * getOrig_swver())){
	...
	/*
	 * List<String> cmdPayloads = null; try{ WifiDevice wifiDevice =
	 * wifiDeviceService.getById(dto.getMac()); if(wifiDevice != null){
	 ...
	 * }finally{ if(cmdPayloads != null){ cmdPayloads.clear(); cmdPayloads =
	 * null; } }
	 */
			...
			sharedNetworksFacadeService.addDevices2SharedNetwork(uid, null, null, false, dmacs,
					new ISharedNetworkNotifyCallback() {
						...
						public void notify(ParamSharedNetworkDTO current, List<String> rdmacs) {
							logger.info(String.format("notify callback uid[%s] rdmacs[%s] sharednetwork conf[%s]", -1,
									rdmacs, JsonHelper.getJSONString(current)));
	...
	public void wifiDeviceSettingChanged(String message) throws Exception {
		...
		if (deviceFacadeService.isURouterDevice(dto.getMac())) {
			...
			/*
			 * //如果需要初始化设备的黑名单配置 按照urouter约定来进行 if(dto.isInit_default_acl()){
			 * WifiDeviceSetting entity =
			 * wifiDeviceSettingService.getById(dto.getMac()); if(entity !=
			 * null){ String modify_urouter_acl =
			 * DeviceHelper.builderDSURouterDefaultVapAndAcl(entity.
			 * getInnerModel()); DaemonHelper.deviceSettingModify(dto.getMac(),
			 * modify_urouter_acl, daemonRpcService); } }
			 */
		}
		ssidModifyWithChangeDeviceName(dto.getMac());

		logger.info(
				String.format("AnsyncMsgBackendProcessor wifiDeviceSettingChanged message[%s] successful", message));
	}

	/**
	 * 当设备的ssid发生变化时 判断设备昵称是否已经被用户改过 如果没有被用户改过 则要随着ssid变化而变化
	 * 
	 * @param mac
	 */
	...
	/**
	 * 根据wifi设备的经纬度调用第三方服务获取地理位置的详细信息 1:记录wifi设备的坐标 (backend) 2:根据坐标提取地理位置详细信息
	 * (backend) 3:增量索引
	 * 
	 * @param message
	 * @throws Exception
	...
	/**
	 * 获取设备VAP下的终端列表 1:更新被管理的终端的上下行速率和ssid bssid 2:更新被管理的终端的限速设置
	 * 
	 * @param message
	 *            modified by Edmond Lee for handset storage
	 */
	 ...
	 * logger.info(String.format(
	 * "AnsyncMsgBackendProcessor WifiDeviceTerminalNotify message[%s]",
	 * message)); WifiDeviceTerminalNotifyDTO dto = JsonHelper.getDTO(message,
	 * WifiDeviceTerminalNotifyDTO.class);
	 * 
	 * //获取设备的配置的dto WifiDeviceSetting setting_entity =
	 * wifiDeviceSettingService.getById(dto.getMac()); if(setting_entity ==
	 * null) return; WifiDeviceSettingDTO setting_entity_dto =
	 * setting_entity.getInnerModel();
	 * 
	 * List<WifiDeviceTerminalDTO> terminals = dto.getTerminals(); if(terminals
	 * != null && !terminals.isEmpty()){ List<String> hdIds = new
	 * ArrayList<String>(); for(WifiDeviceTerminalDTO terminal : terminals){
	 * hdIds.add(terminal.getMac()); }
	 * 
	 * //1:更新被管理的终端的上下行速率和ssid bssid int cursor = 0; List<HandsetDeviceDTO>
	 * handsets = HandsetStorageFacadeService.handsets(hdIds);
	 * //List<HandsetDevice> need_inserts = null; //List<HandsetDevice>
	 * need_updates = null; for(HandsetDeviceDTO handset : handsets){
	 * WifiDeviceTerminalDTO terminal = terminals.get(cursor); //判断是否在黑名单中
	 * if(DeviceHelper.isAclMac(terminal.getMac(), setting_entity_dto))
	 * continue; //匹配终端是否在限速列表中 // WifiDeviceSettingRateControlDTO rc =
	 * DeviceHelper.matchRateControl( // setting_entity_dto, terminal.getMac());
	 * 
	 * if(handset == null){ HandsetDevice hd_entity = new HandsetDevice();
	 * hd_entity.setId(terminal.getMac()); hd_entity.setLast_login_at(new
	 * Date()); hd_entity.setLast_wifi_id(dto.getMac());
	 * hd_entity.setVapname(terminal.getVapname()); hd_entity.setOnline(true);
	 * if(need_inserts == null) need_inserts = new ArrayList<HandsetDevice>();
	 * need_inserts.add(hd_entity); handset = new HandsetDeviceDTO();
	 ...
	 * //handset.setDhcp_name(terminal.getVapname()); }else{
	 ...
	 * entity.setOnline(true); //匹配终端是否在限速列表中 // if(rc != null){ //
	 * entity.setData_tx_limit(rc.getTx()); //
	 * entity.setData_rx_limit(rc.getRx()); // } if(need_updates == null)
	 * need_updates = new ArrayList<HandsetDevice>(); need_updates.add(entity);
	 * }
	 * 
	 * WifiDeviceHandsetPresentSortedSetService.getInstance().addOnlinePresent(
	 * dto.getMac(), terminal.getMac(),
	 * StringUtils.isEmpty(terminal.getData_tx_rate()) ? 0d :
	 * Double.parseDouble(terminal.getData_tx_rate())); cursor++; }
	 * HandsetStorageFacadeService.handsetsComming(handsets); if(need_inserts !=
	 * null) handsetDeviceService.insertAll(need_inserts); if(need_updates !=
	 * null) handsetDeviceService.updateAll(need_updates); }
	 * logger.info(String.format(
	 * "AnsyncMsgBackendProcessor WifiDeviceTerminalNotify message[%s] successful"
	 * , message));
	 * 
	 * }
	 */
	 ...
	 * hdIds.add(terminal.getMac()); }
	 * 
	 * //1:更新被管理的终端的上下行速率和ssid bssid int cursor = 0; List<HandsetDevice> entitys
	 * = handsetDeviceService.findByIds(hdIds, true, true);
	 * //List<WifiHandsetDeviceMark> entitys =
	 * wifiHandsetDeviceMarkService.findByIds(mark_ids, true, true);
	 * List<HandsetDevice> need_inserts = null; List<HandsetDevice> need_updates
	 * = null; for(HandsetDevice entity : entitys){ WifiDeviceTerminalDTO
	 * terminal = terminals.get(cursor); //判断是否在黑名单中
	 * if(DeviceHelper.isAclMac(terminal.getMac(), setting_entity_dto))
	 * continue;
	 * 
	 * //匹配终端是否在限速列表中 // WifiDeviceSettingRateControlDTO rc =
	 * DeviceHelper.matchRateControl( // setting_entity_dto, terminal.getMac());
	 * 
	 * if(entity == null){ HandsetDevice hd_entity = new HandsetDevice();
	 * hd_entity.setId(terminal.getMac()); hd_entity.setLast_login_at(new
	 * Date()); hd_entity.setLast_wifi_id(dto.getMac());
	 * hd_entity.setVapname(terminal.getVapname()); hd_entity.setOnline(true);
	 * 
	 * // WifiHandsetDeviceMark insert_entity = new WifiHandsetDeviceMark(); //
	 * insert_entity.setId(new WifiHandsetDeviceMarkPK(dto.getMac(),
	 * terminal.getMac())); // insert_entity.setVapname(terminal.getVapname());
	 * // insert_entity.setData_tx_rate(terminal.getData_tx_rate()); //
	 * insert_entity.setData_rx_rate(terminal.getData_rx_rate());
	 * 
	 * // if(rc != null){ // insert_entity.setData_tx_limit(rc.getTx()); //
	 * insert_entity.setData_rx_limit(rc.getRx()); // } if(need_inserts == null)
	 * need_inserts = new ArrayList<HandsetDevice>();
	 * need_inserts.add(hd_entity); }else{
	 * entity.setVapname(terminal.getVapname());
	 * entity.setData_tx_rate(terminal.getData_tx_rate());
	 * entity.setData_rx_rate(terminal.getData_rx_rate()); //
	 * entity.setOnline(true); //匹配终端是否在限速列表中 // if(rc != null){ //
	 ...
	 * 
	 * if(need_updates == null) need_updates = new ArrayList<HandsetDevice>();
	 ...
	 * Double.parseDouble(terminal.getData_tx_rate())); cursor++; }
	 * 
	 * if(need_inserts != null) handsetDeviceService.insertAll(need_inserts);
	 * if(need_updates != null) handsetDeviceService.updateAll(need_updates); }
	 * logger.info(String.format(
	 * "AnsyncMsgBackendProcessor WifiDeviceTerminalNotify message[%s] successful"
	 * , message));
	 * 
	 ...
	 * logger.info(String.format("wifiCmdDownNotifyHandle message[%s]",
	 * message)); WifiCmdNotifyDTO dto = JsonHelper.getDTO(message,
	 * WifiCmdNotifyDTO.class); DaemonHelper.daemonCmdDown(dto.getMac(),
	 * dto.getPayload(), daemonRpcService);
	 * //daemonRpcService.wifiDeviceCmdDown(null, dto.getMac(),
	 * dto.getPayload()); logger.info(String.format(
	 * "wifiCmdDownNotifyHandle message[%s] successful", message)); }
	 */
	...
	public void wifiMultiCmdsDownNotifyHandle(String message) {
		...
		if (dto.getDownCmds() == null || dto.getDownCmds().isEmpty())
			return;
	...
	public void deviceModifySettingAalias(String message) {
		...
		if (!StringUtils.isEmpty(mac)) {
			Map<String, List<WifiDeviceSettingMMDTO>> mm_dto_map = JsonHelper.getDTOMapKeyList(content,
					WifiDeviceSettingMMDTO.class);
			if (mm_dto_map != null && !mm_dto_map.isEmpty()) {
				...
				if (mm_incr_dtos != null && !mm_incr_dtos.isEmpty()) {
					for (WifiDeviceSettingMMDTO mm_incr_dto : mm_incr_dtos) {
						WifiDeviceHandsetAliasService.getInstance().hsetHandsetAlias(uid, mm_incr_dto.getMac(),
								mm_incr_dto.getName());
					}
				}
	...
	/**
	 * 修改VAP内容的后续操作 如果修改了ssid，则判断是否需要修改设备昵称
	 * 
	 * @param message
	 */
	...
	// logger.info(String.format("AnsyncMsgBackendProcessor
	// wifiDeviceSettingNotify message[%s]", message));
	// WifiDeviceSettingNotifyDTO dto = JsonHelper.getDTO(message,
	// WifiDeviceSettingNotifyDTO.class);
	//
	// List<String> vapnames = dto.getVapnames();
	// DaemonHelper.deviceTerminalsQuery(dto.getMac(), vapnames,
	// daemonRpcService);
	// /*if(vapnames != null && !vapnames.isEmpty()){
	// List<String> cmds =
	// CMDBuilder.builderDeviceTerminalsQueryWithAutoTaskid(dto.getMac(),
	// dto.getVapnames());
	// daemonRpcService.wifiDeviceCmdsDown(null, dto.getMac(), cmds);
	// }*/
	// logger.info(String.format("AnsyncMsgBackendProcessor
	// wifiDeviceSettingNotify message[%s] successful", message));
	//
	// }
	...
	public void wifiDeviceRealtimeRateFetch(String message) {
		...
		// DaemonHelper.daemonCmdDown(dto.getMac(), dto.getPayload(),
		// daemonRpcService);
		// daemonRpcService.wifiDeviceCmdDown(null, dto.getMac(),
		// dto.getPayload());
	 ...
	 * logger.info(String.format("wifiDevicePeakRateFetch message[%s]",
	 * message)); WifiDeviceSpeedFetchDTO dto = JsonHelper.getDTO(message,
	 * WifiDeviceSpeedFetchDTO.class);
	 * //DaemonHelper.daemonCmdDown(dto.getMac(), dto.getPayload(),
	 * daemonRpcService); //daemonRpcService.wifiDeviceCmdDown(null,
	 * dto.getMac(), dto.getPayload());
	 * //DaemonHelper.deviceRateQuery(dto.getMac(), daemonRpcService);
	 * DaemonHelper.deviceSpeedQuery(dto.getMac(), dto.getType(),
	 * dto.getPeriod(), dto.getDuration(), daemonRpcService);
	 * //WifiDeviceRealtimeRateStatisticsStringService.getInstance().
	 * addPeakRateWaiting(dto.getMac()); logger.info(String.format(
	 * "wifiDevicePeakRateFetch message[%s] successful", message)); }
	 */
	...
	/**
	 * 用户绑定设备之后 会进行下发配置操作 下发获取配置，获取设备测速，设备实时速率, 设备终端列表
	 * 
	 * @param message
	 */
	...
	/**
	 * 用户通过其他方式解绑设备或绑定设备以后 需要对设备的绑定配置数据进行同步
	 * 
	 * @param mac
	 *            设备mac
	 * @param uid
	...
	public void userDeviceBindOperateSyskeySync(String mac, Integer uid) {
		...
		String cmdPayload = CMDBuilder.builderDeviceSettingModify(mac, 0,
				DeviceHelper.builderDSKeyStatusOuter(syskey_dto));
	...
	/**
	 * 设备被用户强制绑定 如果设备是urouter设备，需要清除用户移动设备push数据 增量索引用户绑定数据更新
	 * 
	 * @param message
	 */
	public void userDeviceForceBind(String message) {
		...
		if (user != null) {
			...
			// wifiDeviceIndexIncrementProcesser.bindUserUpdIncrement(dto.getMac(),
			// user);
			// wifiDeviceStatusIndexIncrementService.bindUserUpdIncrement(dto.getMac(),
			// user);
		}
	...
	/**
	 * 用户登录after 根据用户管理的设备 下发获取配置，获取设备测速，设备实时速率, 设备终端列表
	 * 
	 * @param message
	 */
	...
	/**
	 * 用户bbs登录 通过发送push消息通知app 安卓设备推送静默发送 ios设备推送通知发送
	 * 
	 * @param message
	 */
	public void userBBSsignedon(String message) {
		// logger.info(String.format("AnsyncMsgBackendProcessor userBBSsignedon
		// message[%s]", message));
	...
	public void afterUserSignedonThenCmdDown(String mac) {
		 ...
		 * isHDRateWaiting(mac)){ //获取设备的终端列表
		 * DaemonHelper.deviceTerminalsRateQuery(mac, daemonRpcService);
		 * WifiDeviceRealtimeRateStatisticsStringService.getInstance().
		 * addHDRateWaiting(mac); }
		 * if(!WifiDeviceRealtimeRateStatisticsStringService.getInstance().
		 * isRateWaiting(mac)){ //获取设备的实时速率 DaemonHelper.deviceRateQuery(mac,
		 * daemonRpcService);
		 * WifiDeviceRealtimeRateStatisticsStringService.getInstance().
		 * addRateWaiting(mac); }
		 */
	...
	public void sendCaptchaCodeNotifyHandle(String message) {
			...
			if (!RuntimeConfiguration.SecretInnerTest) {
				...
				if (!BusinessRuntimeConfiguration.isSystemNoneedCaptchaValidAcc(mobileWithCountryCode)) {
					...
					if (dto.getCountrycode() == PhoneHelper.Default_CountryCode_Int) {
						...
						String smsg = String.format(BusinessRuntimeConfiguration.InternalCaptchaCodeSMS_Template,
								dto.getCaptcha());
						String response = SmsSenderFactory
								.buildSender(BusinessRuntimeConfiguration.InternalCaptchaCodeSMS_Gateway)
								.send(smsg, dto.getAcc());
						// String response = ChanzorSMSHelper.postSendMsg(smsg,
						// dto.getAcc());
						// String response = WangjianSMSHelper.postSendMsg(smsg,
						// new String[]{dto.getAcc()});
						// logger.info("CaptchaCodeNotifyActHandler Guodu
						// msg:"+message);
						...
						/*
						 * if(dto.getCountrycode() ==
						 * NexmoSMSHelper.UsAndCanada_CountryCode_Int){ String
						 * response = NexmoSMSHelper.send(NexmoSMSHelper.
						 * Default_UsANDCanada_SMS_FROM,mobileWithCountryCode,
						 * String.format(RuntimeConfiguration.
						 * ForeignCaptchaCodeSMS_Template,dto.getCaptcha()));//.
						 * postSendMsg(String.format(RuntimeConfiguration.
						 * InternalCaptchaCodeSMS_Template, dto.getCaptcha()),
						 * new String[]{dto.getAcc()}); logger.info(
						 * "to US and Canada CaptchaCodeNotifyActHandler Nexmo res:"
						 * +response); }else{ String response =
						 * NexmoSMSHelper.send(mobileWithCountryCode,
						 * String.format(RuntimeConfiguration.
						 * ForeignCaptchaCodeSMS_Template,dto.getCaptcha()));//.
						 * postSendMsg(String.format(RuntimeConfiguration.
						 * InternalCaptchaCodeSMS_Template, dto.getCaptcha()),
						 * new String[]{dto.getAcc()}); logger.info(
						 * "to Other CaptchaCodeNotifyActHandler Nexmo res:"
						 * +response); } //logger.info(
						 * "CaptchaCodeNotifyActHandler Nexmo msg:"+message);
						 */ }
				}
			}
		} catch (Exception ex) {
		...
		// logger.info(String.format("sendCaptchaCodeNotifyHandle message[%s]
		// successful", message));

	}
	/*
	 * public void deviceAsyncCmdGen(String message){ logger.info(String.format(
	 * "deviceAsyncCmdGen message[%s]", message)); Set<String> totalDevices =
	 * null; WifiDeviceGroup dgroup = null; List<String> onlineDevices = null;
	 * try{ WifiDeviceAsynCmdGenerateDTO dto = JsonHelper.getDTO(message,
	 * WifiDeviceAsynCmdGenerateDTO.class); totalDevices = new
	 * HashSet<String>(); if(StringUtils.isNotEmpty(dto.getMac()))
	 * totalDevices.add(dto.getMac()); if(dto.getGid() > 0){ dgroup =
	 * wifiDeviceGroupService.getById(dto.getGid()); if(dgroup != null){
	 * if(!dto.isDependency()){ totalDevices.addAll(dgroup.getInnerModels());
	 * }else{ List<WifiDeviceGroup> allByPath =
	 * wifiDeviceGroupService.fetchAllByPath(dgroup.getPath(), true);
	 * for(WifiDeviceGroup _dgroup:allByPath){
	 * totalDevices.addAll(_dgroup.getInnerModels()); } } } }
	 * if(totalDevices.isEmpty()) return; //只给在线的设备发送指令 onlineDevices =
	 * wifiDeviceService.filterOnlineIdsWith(new
	 * ArrayList<String>(totalDevices), true); if(onlineDevices.isEmpty()){
	 * return; } for(String wifi_id:onlineDevices){ try{ WifiDeviceDownTask
	 * downTask = taskFacadeService.apiTaskGenerate(dto.getUid(), wifi_id,
	 * dto.getOpt(), dto.getSubopt(), dto.getExtparams(), dto.getChannel(),
	 * dto.getChannel_taskid()); DaemonHelper.daemonCmdDown(wifi_id,
	 * downTask.getPayload(), daemonRpcService);
	 * }catch(BusinessI18nCodeException bex){ bex.printStackTrace(System.out);
	 * logger.error("TaskGenerate invoke exception : " + bex.getMessage(), bex);
	 * }catch(Exception ex){ ex.printStackTrace(System.out); logger.error(
	 * "TaskGenerate invoke exception : " + ex.getMessage(), ex); } } }finally{
	 * if(onlineDevices != null){ onlineDevices.clear(); onlineDevices = null; }
	 * if(totalDevices != null){ totalDevices.clear(); totalDevices = null; }
	 * if(dgroup != null){ dgroup = null; } }
	 * 
