package com.bhu.vas.business.asyn.web.model;

import com.bhu.vas.business.asyn.web.builder.ActionDTO;
import com.bhu.vas.business.asyn.web.builder.ActionMessageType;

public class UserRegisteredDTO extends ActionDTO {
	//private Integer inviteuid;
	private String channel;
	private String d;
	private String remoteip;
	/*public String getInvitetoken() {
		return invitetoken;
	}

	public void setInvitetoken(String invitetoken) {
		this.invitetoken = invitetoken;
	}

	public Integer getInviteuid() {
		return inviteuid;
	}

	public void setInviteuid(Integer inviteuid) {
		this.inviteuid = inviteuid;
	}*/

	public String getD() {
		return d;
	}

	public String getChannel() {
		return channel;
	}

	public void setChannel(String channel) {
		this.channel = channel;
	}

	public void setD(String d) {
		this.d = d;
	}

	@Override
	public String getActionType() {
		return ActionMessageType.REGISTERED.getPrefix();
	}

	public String getRemoteip() {
		return remoteip;
	}

	public void setRemoteip(String remoteip) {
		this.remoteip = remoteip;
	}
	
	
}
