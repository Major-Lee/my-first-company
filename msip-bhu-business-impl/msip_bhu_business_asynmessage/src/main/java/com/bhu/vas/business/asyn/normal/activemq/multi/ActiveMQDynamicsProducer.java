...
public class ActiveMQDynamicsProducer{
	private static final Logger logger = LoggerFactory.getLogger(ActiveMQDynamicsProducer.class);
	//@Resource
	//private ActiveMQDynamicService activeMQDynamicService;
	@PostConstruct
	public void initialize(){
		//初始化ActiveMQConnectionManager
		ActiveMQConnectionsManager.getInstance().initProducerQueues();
	}
	...
	public void deliverMessage(String Sender_KEY,String message){
		Session des_session = ActiveMQConnectionsManager.getInstance().getSession(Sender_KEY);
		if(des_session == null){
		...
		MessageProducer des_producer = ActiveMQConnectionsManager.getInstance().getProducer(Sender_KEY);
	...
	public boolean initTestProducers(){
		ActiveMQConnectionsManager.getInstance().initConsumerTestProducers();
		return !ActiveMQConnectionsManager.getInstance().currentTestProducerKeys().isEmpty();
	}
	
	public String randomTestProducerKey(){
		return RandomPicker.pick(ActiveMQConnectionsManager.getInstance().currentTestProducerKeys());
	}
	
	public String randomProducerKey(){
		return RandomPicker.pick(ActiveMQConnectionsManager.getInstance().currentProducerKeys());
	}
	
	
	public void deliverTestMessage(String Sender_KEY,String message){
		Session des_session = ActiveMQConnectionsManager.getInstance().getSession(Sender_KEY);
		MessageProducer des_producer = ActiveMQConnectionsManager.getInstance().getTestProducer(Sender_KEY);
		if(des_session == null){
