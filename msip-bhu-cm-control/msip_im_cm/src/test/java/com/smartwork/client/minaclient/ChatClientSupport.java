...
public class ChatClientSupport {
    ...
    public ChatClientSupport(String name, IoHandler handler, Callback callback) {
        if (name == null) {
    ...
    public boolean connect(SocketAddress address,boolean useSsl) {
    	...
    	return connect(connector, address, useSsl);
    }
    ...
    public void login() {
    	Message message = MessageBuilder.builderCliSignonRequestMessage(name, null, "1234");
    	session.write(JsonHelper.getJSONString(message));
    }

    public void broadcast(String textmessage) {
    	if(textmessage.indexOf(":") != -1){
    		String[] messages = textmessage.split(":");
    		//Message message = MessageBuilder.builderCliPeerMediaMessage(name,messages[0],"2014070415hmm.jpg",ChatMediaMessage.Type_Text, messages[1]);//builderCliSignonRequestMessage(name, null, "1234");
    		//session.write(JsonHelper.getJSONString(message));
    	}else{
