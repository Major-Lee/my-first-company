...
public class IMClientSupport {
    ...
    Callback callback;
    Socket conn;
    public IMClientSupport(String name) {
        if (name == null) {
    ...
    public boolean connect(String host, int port){
    	return connect(new InetSocketAddress(host, port), false);
    }
    ...
    public boolean connect(InetSocketAddress address,boolean useSsl) {
		...
		try {
            ...
            this.callback = new IMCallback(name, socketWriter);
            new Thread(new IMMessageRunnable(name, socketReader,callback)).start();
            login();
		} catch (IOException e) {
			e.printStackTrace();
			return false;
...
/*        if (session != null && session.isConnected()) {
    ...
    public void login() {
    	String to = "100174";
    	String cliSignon = JsonHelper.getJSONString(MessageBuilder.builderCliSignonRequestMessage(name, to, null));
    	System.out.println("im client send login : " + cliSignon);
    	//socketWriter.write("LOGIN " + name+"\n");
    	socketWriter.write(cliSignon+"\n");
    	socketWriter.flush();
    }
