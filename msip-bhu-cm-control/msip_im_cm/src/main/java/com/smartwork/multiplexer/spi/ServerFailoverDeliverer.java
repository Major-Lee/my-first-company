...
public class ServerFailoverDeliverer implements PacketDeliverer {
    ...
    public void deliver(String stanza) {
    	...
    	message.setE(true);
    	Session session =  SessionManager.getInstance().getSession(message.getFrom());
		if(session != null && !session.isClosed()){//在线 online
			session.deliver(JsonHelper.getJSONString(message));
		}else{//offline
			//TODO:如果from用户下线，则存入此消息到用户offline error列表
			//UserOfflineMessageService.getInstance().userOfflineLPush(message.getFrom(), BusinessKeyDefine.Present.UserErrorType_DeliverPrefixKey, stanza);
    		System.out.println("user offline:"+stanza);
			return;
		}
    	
    	String streamID = null;
    	//TODO got the streamID from stanza
