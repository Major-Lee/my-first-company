...
import com.smartwork.multiplexer.task.CloseSessionTask;
import com.smartwork.multiplexer.task.DeliveryFailedTask;
import com.smartwork.multiplexer.task.NewSessionTask;
import com.smartwork.multiplexer.task.RouteTask;
import com.smartwork.multiplexer.task.SessionAuthenticatedTask;
 ...
 */
public class ServerSurrogate {//implements IServerSurrogate{
    ...
    public void start() {
    	dispatcherEnabled = JingGlobals.getBooleanProperty("cm.dispatcher.enable", false);//.getIntProperty("cm.dispatcher.port", DEFAULT_MULTIPLEX_PORT);
	                ...
	                while (true) {
	                    try {
	                        ...
	                        for (ConnectionWorkerThread thread : serverConnections.values()) {
	                            thread.getConnection().deliver(MessageBuilder.builderHeartBeatMessage());//.deliverRawText("HB"+"\n");
	                        }
	                    }
     ...
     */
    public void clientSessionCreated(final String streamID,String payload, final InetAddress address) {
    	if(dispatcherEnabled)
    		threadPool.execute(new NewSessionTask(streamID, address));
    }

    public void clientSessionAuthenticated(final String streamID, final InetAddress address) {
    	if(dispatcherEnabled)
    		threadPool.execute(new SessionAuthenticatedTask(streamID, address));
    }
     ...
     */
    public void clientSessionClosed(final String streamID,String payload) {
...
/*    public void setSASLMechanisms(Element mechanisms) {
    ...
    private void createThreadPool() {
        int maxConnections = JingGlobals.getIntProperty("cm.dispatcher.connections", 5);
	...
	/*@Override
	public void clientSessionCreated(String streamID, String payload,
			InetAddress address) {
	}

	@Override
	public void clientSessionClosed(String streamID, String payload) {
