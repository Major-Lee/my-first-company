...
import com.smartwork.im.utils.DNSUtil;
...
public class ConnectionWorkerThread extends Thread {
	private static final Logger logger = LoggerFactory.getLogger(ConnectionWorkerThread.class);
    /**
    ...
    public ConnectionWorkerThread(ThreadGroup group, Runnable target, String name, long stackSize) {
        ...
        CMServer server = CMServer.getInstance();
        this.serverName = server.getServerInfo().getMark();//.getServerName();
        this.managerName = server.getServerInfo().getHn();
    ...
    private boolean createConnection() {
        ...
        int port =
                JingGlobals.getIntProperty("cm.dispatcher.port", DEFAULT_MULTIPLEX_PORT);
        Socket socket = new Socket();
        if (JingGlobals.getXMLProperty("cm.dispatcher.hostname") != null) {
            String hostname = JingGlobals.getXMLProperty("cm.dispatcher.hostname");
            
            // Use the specified hostname and port to connect to the server
            try {
            	logger.info("CM - Trying to connect to server at " + hostname + ":" + port);
                // Establish a TCP connection to the Receiving Server
                socket.connect(new InetSocketAddress(hostname, port), 20000);
                logger.info("CM - Plain connection to server at " + hostname + ":" + port + " successful");
            } catch (IOException e) {
            	logger.error("Error trying to connect to server at " + hostname + ":" + port, e);
                if(socket != null){
            		try {
						socket.close();
            ...
            try {
            	...
            	String domainname = JingGlobals.getXMLProperty("cm.dispatcher.domain");
                DNSUtil.HostAddress address = DNSUtil.resolveXMPPServerDomain(domainname, port);
                realHostname = address.getHost();
                logger.info("CM - Trying to connect to " + serverName + ":" + port +
                        "(DNS lookup: " + realHostname + ":" + port + ")");
                // Establish a TCP connection to the Receiving Server
                socket.connect(new InetSocketAddress(realHostname, port), 20000);
                logger.info("CM - Plain connection to " + serverName + ":" + port + " successful");
            }
            catch (Exception e) {
            	logger.error("Error trying to connect to server: " + serverName +
        ...
        try {
            ...
            String authpwd = JingGlobals.getXMLProperty("cm.dispatcher.password");
            String loginmsg = JsonHelper.getJSONString(MessageBuilder.builderCmRegisterRequestMessage(null, null, this.serverName, authpwd));
            connection.deliver(loginmsg);
            ...
            //InputStreamReader ireader = new InputStreamReader(socket.getInputStream(),CHARSET);
            
            //StringPacketReader reader =  new StringPacketReader();
            //BufferedReader reader = new StringPacketReader().createReader(socket.getInputStream(), CHARSET);
            StringPacketReader reader = new StringPacketReader();
            reader.setInput(new BufferedReader(new InputStreamReader(socket.getInputStream(), CHARSET)));
            //BufferedReader reader = new StringPacketReader().createReader(socket.getInputStream(),CHARSET);
           ...
           /* String id = "";
        	...
        	logger.error("Handshake error while connecting to server: " + serverName +
                    "(DNS lookup: " + realHostname + ":" + port + ")", e);
        }
        catch (Exception e) {
        	logger.error("Error while connecting to server: " + serverName + "(DNS lookup: " +
                    realHostname + ":" + port + ")", e);
        }
 ...
 /*   private boolean secureConnection(XMPPPacketReader reader, StringBuilder openingStream)
    ...
    /*private boolean doHandshake(String streamID, StringPacketReader reader) throws Exception {
        String password = JingGlobals.getXMLProperty("xmpp.password");
        if (password == null) {
        ...
        String response = reader.parseContent();
        if (response != null && response.indexOf("handshake")!=-1) {
...
/*    private int[] decodeVersion(String version) {
     ...
     */
    private void createSocketReader(StringPacketReader reader) {
    ...
    /**
     * Sends a notification to the main server that a new client session has been Authenticated.
     *
     * @param streamID the stream ID assigned by the connection manager to the new session.
     * @param address the remote address of the client.
     */
    public void clientSessionAuthenticated(String streamID, InetAddress address) {
    	String msg = JsonHelper.getJSONString(
    			MessageBuilder.builderCmClientSessionAuthenticatedNotifyRequestMessage(null, null, serverName,streamID, null,
    ...
    public void clientSessionClosed(String streamID) {
        ...
        String msg = JsonHelper.getJSONString(MessageBuilder.builderCmClientSessionClosedNotifyRequestMessage(null, null, serverName,streamID, null));
    	connection.deliver(msg);
    }
     ...
     */
    public void deliveryFailed(String stanza, String streamID) {
    	//在此函数里给消息Message 的e属性设置true
    	Message message = JsonHelper.getDTO(stanza, Message.class);
    	message.setE(true);
    	connection.deliver(JsonHelper.getJSONString(message));
    }
    ...
    public void run() {
        try {
        	...
        	CMServer.getInstance().getServerSurrogate().serverConnections.remove(getName());
            // Close the connection
            connection.close();
        }
    }
    ...
    public void deliver(String stanza, String streamID) {
        ...
        connection.deliver(stanza);//sb.toString());
    }
}
