 ...
 */
public class JingGlobals {
	private static final Logger logger = LoggerFactory.getLogger(JingGlobals.class);
    private static String JING_CONFIG_FILENAME = "/conf" + File.separator + "manager.xml";
    ...
    public static List<String> getXMLProperties(String parent) {
        ...
        // jiveHome not loaded?
        if (xmlProperties == null) {
            return Collections.emptyList();//.EMPTY_LIST;
        }
    ...
    */
    public static void setConfigName(String configName) {
        JING_CONFIG_FILENAME = configName;
    }
    ...
    private synchronized static void loadSetupProperties() {
    	...
    	if (xmlProperties == null) {
            ...
            // Create a manager with the full path to the xml config file.
            	...
            	String paramConfName = System.getProperty("instance.conf");
            	System.out.println("~~~~~~~~~~~~~~~~:"+paramConfName);
            	if(paramConfName != null && !"".equals(paramConfName)){
            		setConfigName(paramConfName);
            	}
                try {
                    xmlProperties = new XMLProperties(home + File.separator + getConfigName());
                    System.out.println("located conf file:"+home + File.separator + getConfigName());
                }
                catch (IOException ioe) {
                	System.out.println(ioe.getMessage());
                	System.out.println("can not locate conf file:"+home + File.separator + getConfigName());
                	System.out.println("load default conf file :classpath:"+JING_CONFIG_FILENAME);
                	InputStream in = null;
                	in = JingGlobals.class.getResourceAsStream(JING_CONFIG_FILENAME);
                	try {
						xmlProperties = new XMLProperties(in);
					} catch (IOException ex) {
						logger.error(ioe.getMessage(),ex);
						failedLoading = true;
					}
                    //failedLoading = true;
                }
