 ...
 * 
 * @author:  张鹏宇
 * @date： 日期：2016-05-05 时间：下午12:15:03
 * @version 1.0
 */
public class BusinessHelperBK extends PropertyEditorSupport {
	 ...
	 * @return
	 */
	public static String generatePaymentReckoningNoByType(String type){
		String randCode = RandomPicker.randString(letters, 4);
		long millisTime =  BusinessHelper.getMillis();
		return type+millisTime+randCode;
	}
	...
	public static String GetBase64ImageStr(String content, String logoPath) throws WriterException, IOException  {//将图片文件转化为字节数组字符串，并对其进行Base64编码处理
			byte[] result = null;
			ByteArrayOutputStream output = null;
			BufferedImage bufferImgage = null;
			try {
				// logoPath = "E:"+File.separator+"logo_big.png";
		...
		return "data:image/png;base64,"+java.util.Base64.getEncoder().encodeToString(result);//返回Base64编码过的字节数组字符串
        //return "data:image/png;base64,"+Base64Helper.encode(result);//返回Base64编码过的字节数组字符串
    }
	private static final int BLACK = Color.BLACK.getRGB();//0xFF66CCCC;
	private static final int WHITE = Color.WHITE.getRGB();//0xFFFFFFFF;
	public static BufferedImage encode2(String contents, int width, int height,String logoPath) {
		...
		try {
			...
			return markeLogo2QRCode(image,logoPath);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	}
	public static BufferedImage markeLogo2QRCode(BufferedImage background,String logoImgPath){
		try{
			//获取画笔
			Graphics2D g = background.createGraphics();
			//读取logo图片
			if(logoImage == null){
				logoImage = ImageIO.read(new File(logoImgPath));
			}
			//BufferedImage logo = ImageIO.read(new File(logoImgPath));
			//设置二维码大小，太大，会覆盖二维码，此处20%
			int logoWidth = logoImage.getWidth(null) > background.getWidth()*2 /10 ? (background.getWidth()*2 /10) : logoImage.getWidth(null);
			int logoHeight = logoImage.getHeight(null) > background.getHeight()*2 /10 ? (background.getHeight()*2 /10) : logoImage.getHeight(null);
			//设置logo图片放置位置
			//中心
			int x = (background.getWidth() - logoWidth) / 2;
			int y = (background.getHeight() - logoHeight) / 2;
			//右下角，15为调整值
...
//	}
    ...
    {//对字节数组字符串进行Base64解码并生成图片
        ...
        {
            //Base64解码
            byte[] b = Base64Helper.decode(imgStr);//.decodeBuffer(imgStr);
